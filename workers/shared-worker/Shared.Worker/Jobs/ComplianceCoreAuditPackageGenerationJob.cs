...
public sealed class ComplianceCoreAuditPackageGenerationJob(
    ILogger<ComplianceCoreAuditPackageGenerationJob> logger,
    ComplianceCoreAuditPackageGenerationClient complianceCoreClient,
    IOptions<ComplianceCoreAuditPackageGenerationOptions> options) : BackgroundService
{
    protected override async Task ExecuteAsync(CancellationToken stoppingToken)
    {
        ...
        if (string.IsNullOrWhiteSpace(settings.ServiceToken))
        {
            logger.LogWarning(
                ...
                $"{ComplianceCoreAuditPackageGenerationOptions.SectionName}:ServiceToken");
            return;
        }
