...
public sealed record StaffArrProcessReadinessRollupsResult(
    int CandidatesFound,
    int RefreshedCount,
    int SkippedCount);

public sealed class StaffArrReadinessRollupClient(
    HttpClient httpClient,
    IOptions<StaffArrReadinessRollupOptions> options)
{
    public async Task<StaffArrProcessReadinessRollupsResult> ProcessBatchAsync(
        CancellationToken cancellationToken)
    {
        var settings = options.Value;
        using var request = new HttpRequestMessage(HttpMethod.Post, "api/internal/readiness-rollups/process-batch");
        ...
        return new StaffArrProcessReadinessRollupsResult(
