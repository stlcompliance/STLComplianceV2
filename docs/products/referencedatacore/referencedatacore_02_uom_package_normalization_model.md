# ReferenceDataCore — Unit of Measure and Package Normalization Model

## Purpose

ReferenceDataCore owns shared unit-of-measure and package normalization so products do not create conflicting conversion logic.
...
- ReferenceDataCore owns shared unit/package normalization that helps those products interpret quantities consistently.
...
SupplyArr may use ReferenceDataCore to normalize vendor package and UOM context.
...
LoadArr may use ReferenceDataCore to normalize scanned receiving quantities and putaway package quantities.

LoadArr still owns inventory balance, stock ledger, receiving transaction, movement transaction, discrepancy, and adjustment.

### MaintainArr

MaintainArr may use ReferenceDataCore to normalize part usage UOM display or attach a known reference entity to a part demand.

MaintainArr still owns work order parts demand, part usage/install record, and return-to-service decision.

### OrdArr

OrdArr may use ReferenceDataCore to normalize requested quantities for orchestration and customer-facing display.
...
ReferenceDataCore supplies the reason and suggested conversion candidates. The owning product owns the blocker.
...
Recommended ReferenceDataCore events:

```text
- referencedatacore.uom.created
- referencedatacore.uom.updated
- referencedatacore.package.created
- referencedatacore.package_conversion.created
- referencedatacore.package_conversion.review_required
- referencedatacore.package_conversion.published
