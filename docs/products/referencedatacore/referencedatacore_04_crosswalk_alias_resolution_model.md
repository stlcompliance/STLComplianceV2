# ReferenceDataCore — Crosswalk, Alias, and Resolution Model
...
ReferenceDataCore owns shared crosswalk and resolution rules.
...
External credentials are governed by integration and security constitutions. ReferenceDataCore should not become a general credential vault.

## Resolution request

A consuming product may ask ReferenceDataCore to resolve an identifier or alias.
...
When sources disagree, ReferenceDataCore should use the configured source-priority policy.
...
ReferenceDataCore records or returns the resolution evidence; the product records the operational decision.
...
- referencedatacore.alias.created
