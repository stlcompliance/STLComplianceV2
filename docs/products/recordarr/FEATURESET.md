...
> **Audit anchor worker reality:** A disabled-by-default audit anchor worker now consumes explicit external anchor manifests for configured tenants and calls the durable audit-seal anchor path only for known unanchored tenant seals. Missing manifests, missing provider evidence, cross-tenant rows, unknown seals, and already anchored seals do not create anchor success; hash mismatches persist failed anchor evidence. External WORM storage, notarization provider scheduling, timestamp-authority lifecycle management, and provider-backed immutable audit retention beyond explicit anchor manifests remain incomplete.
...
- The disabled audit anchor worker reduces the immutable-audit orchestration gap for explicit provider anchor manifests, but provider-side notarization scheduling, WORM retention, timestamp-authority lifecycle management, and immutable audit retention automation remain release blockers.
...
- Configured audit anchor manifest worker outcomes are also verifiable through tenant-scoped audit-seal anchor evidence.
