# STL Compliance Event Catalog and Consumer Matrix

## Purpose

This document provides a starter event catalog for implementation.
...
|---|---|---|---|
| `nexarr.membership.activated` | NexArr | Product APIs, ReportArr, Field Companion | establish active tenant context |
| `nexarr.membership.suspended` | NexArr | Product APIs, ReportArr, Field Companion | revoke tenant work context and sessions |
| `nexarr.product_operational_state.changed` | NexArr | suite shell, product APIs, ReportArr | update launcher health/degraded state |
...
| `platform.reference_data.dataset_version.published` | Platform Reference Data service | SupplyArr, LoadArr, MaintainArr, ReportArr | refresh reference cache |
| `platform.reference_data.reference_entity.merged` | Platform Reference Data service | consuming products | stale reference review |
| `platform.reference_data.crosswalk.review_required` | Platform Reference Data service | source product | mapping review |
...
LedgArr events publish financial state after LedgArr has validated tenant/session or service context, source references, Financial Legal Entity assignment, fiscal period rules, mapping, and posting controls.
