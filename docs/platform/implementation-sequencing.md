...
This document defines a practical build and remediation order that respects product ownership boundaries and closes the audited production, security, durability, and UI gaps.
...
Before feature expansion or production release:
...
```text
- Product registry contains only user-facing products; Platform Reference Data remains a platform service and administration utility.
- Existing platform constitutions are present.
- Duplicate compiled docs are not treated as canonical over granular docs.
```

## Phase 0A — Audit-blocker remediation

Before expanding features:

```text
- authorize and tenant-scope every AssurArr route
- replace global/in-memory production stores in RecordArr, OrdArr, and ReportArr
- replace LoadArr fixtures, no-op writes, and local-success fallbacks
- repair deterministic CI/nightly browser failures
- move browser session security to the SPA hardening constitution
- enforce semantic theme tokens and shared page archetypes
- add cross-tenant, permission-denial, restart, and truthful-error tests
```
...
- no fixture/demo/no-op path is reachable as production truth
- every route is represented in an authorization map
- every primary record has required page archetypes and designed states
- light and dark visual smoke tests pass across all product shells
```
...
- all-product launcher and operational-state directory
- product launch/handoff
- session, refresh-token, and service-client hardening
...
Platform Reference Data service (non-product platform service)
- dataset model
- lookup API
...
- Platform Reference Data service supports item identity/UOM lookup.
