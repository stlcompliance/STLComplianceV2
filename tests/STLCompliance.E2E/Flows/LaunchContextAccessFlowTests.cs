...
/// <summary>
/// M13 ship-gate: ordinary product APIs stay accessible when the JWT uses a non-product launch context,
/// while Compliance Core studio and unknown-product launch paths remain denied appropriately.
/// </summary>
[Trait("Category", "Integration")]
[Trait("Area", "AccessModel")]
public sealed class LaunchContextAccessFlowTests : IAsyncLifetime
{
    private static readonly string[] NonProductLaunchContext = ["nexarr"];
    ...
    public async Task InitializeAsync()
    {
        ...
        _supplyarrFactory = CreateProductFactory<global::SupplyArr.Api.Program, SupplyArr.Api.Data.SupplyArrDbContext>(
            $"E2E-LaunchContext-SupplyArr-{Guid.NewGuid():N}",
            signingKey,
            nexarrBaseUrl);
        ...
        _reportarrFactory = CreateProductFactory<global::ReportArr.Api.Program, ReportArr.Api.Data.ReportArrDbContext>(
            $"E2E-LaunchContext-ReportArr-{Guid.NewGuid():N}",
            signingKey,
            nexarrBaseUrl);
        ...
        _complianceCoreFactory = CreateProductFactory<global::ComplianceCore.Api.Program, ComplianceCore.Api.Data.ComplianceCoreDbContext>(
            $"E2E-LaunchContext-ComplianceCore-{Guid.NewGuid():N}",
            signingKey,
            nexarrBaseUrl);
    ...
    public static TheoryData<M13OrdinaryProductAccessProbe> ProductOrdinaryAccessProbes =>
        new(StlM13ShipGateCatalog.ProductApiOrdinaryAccessProbes);

    [Theory]
    [MemberData(nameof(ProductOrdinaryAccessProbes))]
    public async Task Ordinary_product_api_me_allows_non_product_launch_context(M13OrdinaryProductAccessProbe probe)
    {
        var token = MintTokenWithNonProductLaunchContext(probe.ProductKey);
        var client = ClientFor(probe.ProductKey);

        var response = await client.SendAsync(
            HttpTestClient.Authorized(HttpMethod.Get, probe.Path, token));

        response.EnsureSuccessStatusCode();
    }

    [Fact]
    public async Task ComplianceCore_studio_me_forbidden_for_non_platform_admin_without_studio_access()
    {
        var token = MintTokenWithNonProductLaunchContext("compliancecore");

        var response = await _complianceCoreClient.SendAsync(
            HttpTestClient.Authorized(HttpMethod.Get, StlM13ShipGateCatalog.ComplianceCoreStudioMePath, token));

        Assert.Equal(HttpStatusCode.Forbidden, response.StatusCode);
    }

    [Fact]
    public async Task NexArr_launch_context_not_found_for_unknown_product_key()
    {
    ...
    [Fact]
    public async Task NexArr_launch_context_allows_ordinary_product_after_compatibility_launch_destination_revocation()
    {
        using (var scope = _nexarr.Factory.Services.CreateScope())
        {
            var db = scope.ServiceProvider.GetRequiredService<NexArrDbContext>();
            var launchDestinationRecord = await db.Entitlements.FirstOrDefaultAsync(e =>
        ...
        var response = await _nexarr.Client.SendAsync(
            HttpTestClient.Authorized(
                HttpMethod.Get,
                $"{StlM13ShipGateCatalog.NexArrLaunchContextPath}?productKey=staffarr",
                loginToken));

        response.EnsureSuccessStatusCode();
    }

    private string MintTokenWithNonProductLaunchContext(string productKey)
    {
        ...
        return productKey switch
        {
            "staffarr" => E2EAccessTokenHelper.StaffArr(
                _staffarrFactory.Services, tenantId, userId, personId, NonProductLaunchContext),
            "trainarr" => E2EAccessTokenHelper.TrainArr(
                _trainarrFactory.Services, tenantId, userId, personId, NonProductLaunchContext),
            "maintainarr" => E2EAccessTokenHelper.MaintainArr(
                _maintainarrFactory.Services, tenantId, userId, NonProductLaunchContext),
            "routarr" => E2EAccessTokenHelper.RoutArr(
                _routarrFactory.Services, tenantId, userId, NonProductLaunchContext),
            "supplyarr" => E2EAccessTokenHelper.SupplyArr(
                _supplyarrFactory.Services, tenantId, userId, NonProductLaunchContext),
            "reportarr" => E2EAccessTokenHelper.ReportArr(
                _reportarrFactory.Services, tenantId, userId, NonProductLaunchContext),
            "compliancecore" => E2EAccessTokenHelper.ComplianceCore(
                _complianceCoreFactory.Services, tenantId, userId, NonProductLaunchContext),
