...
public sealed class SupplyArrVendorCatalogApiTests : IAsyncLifetime
{
    ...
    public async Task InitializeAsync()
    {
        ...
        var nexArrDbName = $"VendorCatalogApiNexArr-{Guid.NewGuid():N}";
        var supplyArrDbName = $"VendorCatalogApiSupplyArr-{Guid.NewGuid():N}";
        ...
        (_vendorPartyId, _partOneId, _partTwoId) = await SeedVendorCatalogAsync();
    }
    ...
    [Fact]
    public async Task Vendor_catalog_api_sync_applies_rows_and_updates_preferred_vendor()
    {
        var response = await _supplyarrClient.SendAsync(
            Authorized(
                HttpMethod.Post,
                "/api/v1/vendor-catalogs/sync",
                _userToken,
                new VendorCatalogApiSyncRequest(
                    "api-vendor",
                    false,
                    [
                        new VendorCatalogApiSyncItem(
                            "filter-001",
                            "NEW-001",
                            true,
                            12.5m,
        ...
        var sync = await response.Content.ReadFromJsonAsync<VendorCatalogApiSyncResponse>();
        Assert.NotNull(sync);
        ...
        var preferredLink = await db.PartVendorLinks.Include(x => x.Part).FirstAsync(x => x.PartId == _partOneId);
        Assert.Equal("NEW-001", preferredLink.VendorPartNumber);
        ...
        var createdLink = await db.PartVendorLinks.Include(x => x.Part).FirstAsync(x => x.PartId == _partTwoId);
        Assert.Equal("NEW-002", createdLink.VendorPartNumber);
    ...
    [Fact]
    public async Task Vendor_catalog_api_sync_dry_run_does_not_persist_changes()
    {
        var response = await _supplyarrClient.SendAsync(
            Authorized(
                HttpMethod.Post,
        ...
        var link = await db.PartVendorLinks.Include(x => x.Part).FirstAsync(x => x.PartId == _partOneId);
        Assert.Equal("OLD-001", link.VendorPartNumber);
    ...
    private async Task<(Guid vendorId, Guid partOneId, Guid partTwoId)> SeedVendorCatalogAsync()
    {
        await using var scope = _supplyarrFactory.Services.CreateAsyncScope();
        var db = scope.ServiceProvider.GetRequiredService<SupplyArrDbContext>();
        ...
        return (vendor.Id, partOne.Id, partTwo.Id);
    }
