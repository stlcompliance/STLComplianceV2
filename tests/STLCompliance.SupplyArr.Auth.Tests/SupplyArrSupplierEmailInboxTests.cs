...
using IngestSupplierEmailInboxRequest = SupplyArr.Api.Contracts.IngestSupplierEmailInboxRequest;
using IngestSupplierEmailInboxResponse = SupplyArr.Api.Contracts.IngestSupplierEmailInboxResponse;
...
using SupplierEmailInboxListResponse = SupplyArr.Api.Contracts.SupplierEmailInboxListResponse;
...
public sealed class SupplyArrSupplierEmailInboxTests : IAsyncLifetime
{
    ...
    public async Task InitializeAsync()
    {
        ...
        var nexArrDbName = $"SupplierEmailNexArr-{Guid.NewGuid():N}";
        var supplyArrDbName = $"SupplierEmailSupplyArr-{Guid.NewGuid():N}";
    ...
    [Fact]
    public async Task Supplier_email_inbox_auto_links_quotes_and_order_confirmations()
    {
        var (supplier, part) = await SeedSupplierAndPartAsync();

        var rfq = await CreateRfqAsync(part.PartId, supplier.SupplierId, "RFQ-EMAIL-001");
        var supplierQuote = await CreateAndSubmitSupplierQuoteAsync(rfq.RfqId, supplier.SupplierId, rfq.Lines[0].LineId);
        await SelectSupplierQuoteAsync(rfq.RfqId, supplierQuote.SupplierQuoteId);
        var awardedRfq = await GetRfqAsync(rfq.RfqId);
        Assert.Equal("awarded", awardedRfq.Status);
        Assert.Equal(supplier.SupplierId, awardedRfq.AwardedSupplierId);

        var quoteEmail = await IngestSupplierEmailAsync(new IngestSupplierEmailInboxRequest(
            "mail-001",
            "quote_received",
        ...
        Assert.Equal(supplier.SupplierKey, quoteEmail.Message.SupplierKey);
        ...
        var orderEmail = await IngestSupplierEmailAsync(new IngestSupplierEmailInboxRequest(
            "mail-002",
        ...
        var listResponse = await _supplyarrClient.SendAsync(Authorized(HttpMethod.Get, "/api/v1/supplier-email-inbox?limit=10", _userToken));
        listResponse.EnsureSuccessStatusCode();
        var inbox = (await listResponse.Content.ReadFromJsonAsync<SupplierEmailInboxListResponse>())!;
    ...
    private async Task<(SupplierResponse Supplier, PartResponse Part)> SeedSupplierAndPartAsync()
    {
        var supplierRequest = Authorized(HttpMethod.Post, "/api/suppliers", _userToken);
        supplierRequest.Content = JsonContent.Create(new CreateSupplierRequest(
            $"supplier-email-{Guid.NewGuid():N}"[..20],
            null,
            null,
            "Supplier Email Supply",
            "Supplier Email Supply LLC",
            null,
            "Supplier for email inbox integration.",
            null,
            null,
            null,
            null,
            ...
            null,
            null));
        var supplierResponse = await _supplyarrClient.SendAsync(supplierRequest);
        supplierResponse.EnsureSuccessStatusCode();
        var supplier = (await supplierResponse.Content.ReadFromJsonAsync<SupplierResponse>())!;

        var contactRequest = Authorized(HttpMethod.Post, $"/api/suppliers/{supplier.SupplierId}/contacts", _userToken);
        ...
        catalogRequest.Content = JsonContent.Create(new CreatePartCatalogRequest(
            ...
            "Email Inbox Catalog",
            "Test catalog for supplier email inbox"));
        ...
        partRequest.Content = JsonContent.Create(new CreatePartRequest(
            ...
            catalog.CatalogId,
            "Email Inbox Part",
            "Test part for supplier email inbox",
            "general",
            "each",
            "Supplier",
            "S-100"));
    ...
    private async Task<RfqResponse> CreateRfqAsync(Guid partId, Guid supplierId, string rfqKey)
    {
        ...
        createRequest.Content = JsonContent.Create(new CreateRfqRequest(
            rfqKey,
            "Supplier email inbox RFQ",
            "RFQ used for supplier email inbox tests",
        ...
        inviteRequest.Content = JsonContent.Create(new InviteRfqSuppliersRequest([supplierId]));
    ...
    private async Task<SupplierQuoteResponse> CreateAndSubmitSupplierQuoteAsync(Guid rfqId, Guid supplierId, Guid rfqLineId)
    {
        ...
        createQuote.Content = JsonContent.Create(new CreateSupplierQuoteRequest(
            ...
            "USD",
            "Supplier inbox quote"));
        var createResponse = await _supplyarrClient.SendAsync(createQuote);
        createResponse.EnsureSuccessStatusCode();
        var quote = (await createResponse.Content.ReadFromJsonAsync<SupplierQuoteResponse>())!;
        ...
        submitResponse.EnsureSuccessStatusCode();
        return (await submitResponse.Content.ReadFromJsonAsync<SupplierQuoteResponse>())!;
    }

    private async Task SelectSupplierQuoteAsync(Guid rfqId, Guid supplierQuoteId)
    {
        var request = Authorized(HttpMethod.Post, $"/api/rfqs/{rfqId}/select-quote", _userToken);
        request.Content = JsonContent.Create(new SelectSupplierQuoteRequest(supplierQuoteId));
    ...
    private async Task<PurchaseRequestResponse> CreatePurchaseRequestFromRfqAsync(Guid rfqId, string requestKey)
    {
        ...
        request.Content = JsonContent.Create(new CreatePurchaseRequestFromRfqRequest(requestKey, "Supplier inbox PR", "Created from inbox RFQ"));
        var response = await _supplyarrClient.SendAsync(request);
        response.EnsureSuccessStatusCode();
    ...
    private async Task<PurchaseOrderResponse> CreatePurchaseOrderAsync(Guid purchaseRequestId, string orderKey)
    {
        ...
        request.Content = JsonContent.Create(new CreatePurchaseOrderFromPurchaseRequestRequest(orderKey, "Supplier inbox PO", "Created from inbox PR"));
        var response = await _supplyarrClient.SendAsync(request);
        response.EnsureSuccessStatusCode();
    ...
    private async Task<IngestSupplierEmailInboxResponse> IngestSupplierEmailAsync(IngestSupplierEmailInboxRequest request)
    {
        var messageRequest = Authorized(HttpMethod.Post, "/api/v1/supplier-email-inbox", _userToken);
        messageRequest.Content = JsonContent.Create(request);
        var response = await _supplyarrClient.SendAsync(messageRequest);
        response.EnsureSuccessStatusCode();
        return (await response.Content.ReadFromJsonAsync<IngestSupplierEmailInboxResponse>())!;
    }
    ...
    private async Task<string> IssueHandoffServiceTokenAsync(string adminToken)
    {
        ...
        registerRequest.Content = JsonContent.Create(new RegisterServiceClientRequest(
            $"supplyarr-supplier-email-{Guid.NewGuid():N}",
            "supplyarr supplier email inbox test",
            "supplyarr",
            ["supplyarr"]));
