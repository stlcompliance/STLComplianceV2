...
public sealed class SupplyArrVendorRestrictionTests : IAsyncLifetime
{
    ...
    public async Task InitializeAsync()
    {
        ...
        var nexArrDbName = $"VendorRestrictionNexArr-{Guid.NewGuid():N}";
        var supplyArrDbName = $"VendorRestrictionSupplyArr-{Guid.NewGuid():N}";
    ...
    [Fact]
    public async Task Vendor_restriction_blocks_purchase_request_then_lift_allows()
    {
        ...
        var createRestrictionRequest = Authorized(
            HttpMethod.Post,
            $"/api/parties/{vendor.PartyId}/vendor-restrictions",
            _userToken);
        ...
        var enforcementResponse = await _supplyarrClient.SendAsync(
            Authorized(HttpMethod.Get, $"/api/parties/{vendor.PartyId}/vendor-restrictions/enforcement", _userToken));
        enforcementResponse.EnsureSuccessStatusCode();
        var enforcement = (await enforcementResponse.Content.ReadFromJsonAsync<VendorRestrictionEnforcementResponse>())!;
        Assert.True(enforcement.IsBlocked);
        Assert.Contains(VendorRestrictionScopes.PurchaseRequests, enforcement.ActiveScopes);
        ...
        var liftRequest = Authorized(
            HttpMethod.Post,
            $"/api/vendor-restrictions/{restriction.RestrictionId}/lift",
            _userToken);
        liftRequest.Content = JsonContent.Create(new LiftVendorRestrictionRequest("Audit cleared"));
        ...
        allowedPrRequest.Content = JsonContent.Create(new CreatePurchaseRequestRequest(
            $"vr-pr-{Guid.NewGuid():N}"[..20],
            "Restricted vendor PR after lift",
            string.Empty,
            vendor.PartyId,
            [new CreatePurchaseRequestLineRequest(part.PartId, 1m, string.Empty)]));
    ...
    [Fact]
    public async Task Create_vendor_restriction_enqueues_outbox_event()
    {
        var vendor = await CreateVendorAsync();
        ...
        var updateApproval = Authorized(
            ...
            _userToken);
        updateApproval.Content = JsonContent.Create(new UpdateExternalPartyApprovalStatusRequest("approved"));
        ...
        var outbox = await db.IntegrationOutboxEvents
            .Where(x => x.EventKind == IntegrationOutboxEventKinds.VendorRestrictionCreated)
            .ToListAsync();
        Assert.NotEmpty(outbox);

        var party = await db.ExternalParties.SingleAsync(x => x.Id == vendor.PartyId);
    ...
    private async Task<string> IssueHandoffServiceTokenAsync(string adminToken)
    {
        ...
        registerRequest.Content = JsonContent.Create(new RegisterServiceClientRequest(
            $"supplyarr-vr-handoff-{Guid.NewGuid():N}",
            "supplyarr vendor restriction handoff test",
            "supplyarr",
            ["supplyarr"]));
