...
public sealed class StaffArrTrainArrPersonTrainingHistoryTests : IAsyncLifetime
{
    ...
    public async Task InitializeAsync()
    {
        ...
        var nexArrDbName = $"TrainHistNexArr-{Guid.NewGuid():N}";
        var staffArrDbName = $"TrainHistStaffArr-{Guid.NewGuid():N}";
        var trainArrDbName = $"TrainHistTrainArr-{Guid.NewGuid():N}";
        ...
        _staffarrFactory = new WebApplicationFactory<global::StaffArr.Api.Program>().WithWebHostBuilder(builder =>
        {
            ...
            builder.ConfigureServices(services =>
            {
                ...
                services.AddHttpClient<global::StaffArr.Api.Services.TrainArrPersonTrainingHistoryClient>()
        ...
        await SeedTrainarrHistoryAsync(_personId);
    }
    ...
    [Fact]
    public async Task Staffarr_reads_trainarr_person_training_history_for_person()
    {
        var adminToken = CreateStaffArrAccessToken(["staffarr"], "tenant_admin");
        var response = await _staffarrClient.SendAsync(
            Authorized(
                HttpMethod.Get,
                $"/api/people/{_personId:D}/trainarr-training-history?limit=10",
                adminToken));
        response.EnsureSuccessStatusCode();

        var history = (await response.Content.ReadFromJsonAsync<TrainarrPersonTrainingHistoryResponse>())!;
        Assert.Equal(_personId, history.PersonId);
        Assert.Equal("trainarr", history.SourceProduct);
        Assert.True(history.TotalCount >= 1);
        Assert.Contains(history.Items, x => x.EventKind == TrainingDomainEventKinds.AssignmentCreated);
    }

    [Fact]
    public async Task Trainarr_integration_endpoint_requires_staffarr_service_token()
    {
        var response = await _trainarrClient.SendAsync(
            Authorized(
                HttpMethod.Get,
                $"/api/integrations/person-training-history?tenantId={PlatformSeeder.DemoTenantId:D}&staffarrPersonId={_personId:D}",
                CreateStaffArrAccessToken(["staffarr"], "tenant_admin")));
    ...
    private async Task SeedTrainarrHistoryAsync(Guid personId)
    {
        ...
        db.PersonTrainingHistoryEntries.Add(new PersonTrainingHistoryEntry
        {
            ...
            Summary = "Training assignment created (report seed)",
        ...
        string actionScope)
    {
        ...
        registerRequest.Content = JsonContent.Create(new RegisterServiceClientRequest(
            $"{sourceProduct}-trainhist-{Guid.NewGuid():N}",
            "train history test",
            sourceProduct,
            allowedProducts));
