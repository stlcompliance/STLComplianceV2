...
public sealed class NexArrFieldCompanionFieldInspectionTests : IAsyncLifetime
{
    ...
    public async Task InitializeAsync()
    {
        ...
        var dbName = $"NexArrFieldCompanionInspection-{Guid.NewGuid():N}";
    ...
    [Fact]
    public async Task Get_fieldcompanion_field_inspection_detail_proxies_to_maintainarr()
    {
        var token = await LoginAsync(PlatformSeeder.DemoAdminEmail);
        var request = Authorized(
            HttpMethod.Get,
            $"/api/fieldcompanion/field-tasks/inspection?taskKey=maintainarr:inspection:{_inspectionRunId:D}",
            token);

        var response = await _client.SendAsync(request);
        response.EnsureSuccessStatusCode();

        var payload = (await response.Content.ReadFromJsonAsync<FieldCompanionFieldInspectionDetailResponse>())!;
    ...
    [Fact]
    public async Task Submit_fieldcompanion_field_inspection_answers_proxies_to_maintainarr()
    {
        var token = await LoginAsync(PlatformSeeder.DemoAdminEmail);
        var checklistItemId = Guid.Parse("33333333-3333-3333-3333-333333333333");
        var request = Authorized(HttpMethod.Post, "/api/fieldcompanion/field-tasks/inspection/answers", token);
        request.Content = JsonContent.Create(new SubmitFieldCompanionFieldInspectionAnswersRequest(
            $"maintainarr:inspection:{_inspectionRunId:D}",
            [
                new FieldCompanionFieldInspectionAnswerInput(checklistItemId, "pass", null, null),
            ]));

        var response = await _client.SendAsync(request);
        response.EnsureSuccessStatusCode();

        var payload = (await response.Content.ReadFromJsonAsync<FieldCompanionFieldInspectionAnswersResponse>())!;
        Assert.Equal("maintainarr", payload.ProductKey);
    ...
    [Fact]
    public async Task Complete_fieldcompanion_field_inspection_proxies_to_maintainarr()
    {
        var token = await LoginAsync(PlatformSeeder.DemoAdminEmail);
        var request = Authorized(HttpMethod.Post, "/api/fieldcompanion/field-tasks/inspection/complete", token);
        request.Content = JsonContent.Create(new CompleteFieldCompanionFieldInspectionRequest(
            $"maintainarr:inspection:{_inspectionRunId:D}"));

        var response = await _client.SendAsync(request);
        response.EnsureSuccessStatusCode();

        var payload = (await response.Content.ReadFromJsonAsync<FieldCompanionFieldInspectionCompleteResponse>())!;
