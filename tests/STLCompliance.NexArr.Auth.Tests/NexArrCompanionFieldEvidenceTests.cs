...
public sealed class NexArrCompanionFieldEvidenceTests : IAsyncLifetime
{
    ...
    public async Task InitializeAsync()
    {
        ...
        var dbName = $"NexArrCompanionEvidence-{Guid.NewGuid():N}";
    ...
    [Fact]
    public async Task Submit_companion_field_evidence_proxies_to_trainarr_assignment()
    {
        var token = await LoginAsync(PlatformSeeder.DemoAdminEmail);
        var request = Authorized(HttpMethod.Post, "/api/companion/field-tasks/evidence", token);
        request.Content = JsonContent.Create(new SubmitCompanionFieldEvidenceRequest(
            $"trainarr:assignment:{_assignmentId:D}",
            CompanionFieldEvidenceCaptureKinds.Photo,
            "site-photo.jpg",
            "image/jpeg",
            Convert.ToBase64String("fake jpeg bytes"u8.ToArray()),
            "Companion field capture"));

        var response = await _client.SendAsync(request);
        response.EnsureSuccessStatusCode();

        var payload = (await response.Content.ReadFromJsonAsync<CompanionFieldEvidenceResponse>())!;
    ...
    public async Task Submit_rejects_unsupported_product_task()
    {
        ...
        request.Content = JsonContent.Create(new SubmitCompanionFieldEvidenceRequest(
            ...
            CompanionFieldEvidenceCaptureKinds.Document,
            "inspection.pdf",
            "application/pdf",
