...
public sealed class NexArrCompanionFieldDvirTests : IAsyncLifetime
{
    ...
    public async Task InitializeAsync()
    {
        ...
        var dbName = $"NexArrCompanionDvir-{Guid.NewGuid():N}";
    ...
    [Fact]
    public async Task Submit_companion_field_dvir_proxies_to_routarr_trip()
    {
        var token = await LoginAsync(PlatformSeeder.DemoAdminEmail);
        var request = Authorized(HttpMethod.Post, "/api/companion/field-tasks/dvir", token);
        request.Content = JsonContent.Create(new SubmitCompanionFieldDvirRequest(
        ...
        var payload = (await response.Content.ReadFromJsonAsync<CompanionFieldDvirResponse>())!;
