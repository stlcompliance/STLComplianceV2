...
public sealed class NexArrCompanionWebPushTests : IAsyncLifetime
{
    private static readonly VapidDetails TestVapid = VapidHelper.GenerateVapidKeys();

    private readonly List<CompanionWebPushSendCall> _pushCalls = [];
    ...
    public async Task InitializeAsync()
    {
        ...
        var nexArrDbName = $"NexArrCompanionWebPush-{Guid.NewGuid():N}";
        ...
        _nexarrFactory = new WebApplicationFactory<global::NexArr.Api.Program>().WithWebHostBuilder(builder =>
        {
            ...
            builder.UseSetting("CompanionWebPush:Subject", "mailto:companion-push@test.local");
            builder.UseSetting("CompanionWebPush:PublicKey", TestVapid.PublicKey);
            builder.UseSetting("CompanionWebPush:PrivateKey", TestVapid.PrivateKey);
            builder.ConfigureServices(services =>
            {
                RemoveDbContext<NexArrDbContext>(services);
                services.AddDbContext<NexArrDbContext>(options => options.UseInMemoryDatabase(nexArrDbName));
                services.AddSingleton<ICompanionWebPushSender>(new RecordingWebPushSender(_pushCalls));
            });
        });
    ...
    [Fact]
    public async Task Subscribe_persists_push_subscription_for_companion_user()
    {
        var session = await RedeemCompanionSessionAsync();
        var subscribeResponse = await _nexarrClient.SendAsync(Authorized(
            HttpMethod.Post,
            "/api/companion/push/subscribe",
            session.AccessToken,
            new UpsertCompanionPushSubscriptionRequest(
                "https://push.example.test/device-1",
                new CompanionPushSubscriptionKeysRequest("p256dh-test-key", "auth-test-key"),
                "vitest")));
        subscribeResponse.EnsureSuccessStatusCode();

        await using var scope = _nexarrFactory.Services.CreateAsyncScope();
        var db = scope.ServiceProvider.GetRequiredService<NexArrDbContext>();
        var row = await db.CompanionPushSubscriptions.SingleAsync(x =>
    ...
    public async Task Field_inbox_refresh_dispatches_web_push_without_webhook()
    {
        ...
        var subscribeResponse = await _nexarrClient.SendAsync(Authorized(
            ...
            new UpsertCompanionPushSubscriptionRequest(
                ...
                new CompanionPushSubscriptionKeysRequest("p256dh-test-key-2", "auth-test-key-2"),
                null)));
        subscribeResponse.EnsureSuccessStatusCode();
    ...
    public async Task Unsubscribe_removes_push_subscription()
    {
        ...
        var subscribeResponse = await _nexarrClient.SendAsync(Authorized(
            ...
            new UpsertCompanionPushSubscriptionRequest(
                endpoint,
                new CompanionPushSubscriptionKeysRequest("p256dh-test-key-3", "auth-test-key-3"),
                null)));
        subscribeResponse.EnsureSuccessStatusCode();
        ...
        var unsubscribeRequest = Authorized(
            ...
            "/api/companion/push/subscribe",
            session.AccessToken);
        unsubscribeRequest.Content = JsonContent.Create(new UnsubscribeCompanionPushRequest(endpoint));
        ...
        await using var scope = _nexarrFactory.Services.CreateAsyncScope();
        var db = scope.ServiceProvider.GetRequiredService<NexArrDbContext>();
        Assert.False(await db.CompanionPushSubscriptions.AnyAsync(x =>
            x.TenantId == PlatformSeeder.DemoTenantId && x.UserId == session.UserId));
    }
    ...
    private sealed record CompanionWebPushSendCall(Guid SubscriptionId, string PayloadJson);

    private sealed class RecordingWebPushSender(List<CompanionWebPushSendCall> calls) : ICompanionWebPushSender
    {
        public Task<CompanionWebPushSendResult> SendAsync(
            CompanionPushSubscription subscription,
            string payloadJson,
            CancellationToken cancellationToken = default)
        {
            calls.Add(new CompanionWebPushSendCall(subscription.Id, payloadJson));
            return Task.FromResult(new CompanionWebPushSendResult(true, 201, null));
        }
    }
}
