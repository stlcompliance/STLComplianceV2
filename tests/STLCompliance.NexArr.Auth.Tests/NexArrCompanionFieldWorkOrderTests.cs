...
public sealed class NexArrCompanionFieldWorkOrderTests : IAsyncLifetime
{
    ...
    public async Task InitializeAsync()
    {
        ...
        var dbName = $"NexArrCompanionWorkOrder-{Guid.NewGuid():N}";
    ...
    [Fact]
    public async Task Get_companion_field_work_order_detail_proxies_to_maintainarr()
    {
        var token = await LoginAsync(PlatformSeeder.DemoAdminEmail);
        var request = Authorized(
            HttpMethod.Get,
            $"/api/companion/field-tasks/work-order?taskKey=maintainarr:work-order:{_workOrderId:D}",
            token);

        var response = await _client.SendAsync(request);
        response.EnsureSuccessStatusCode();

        var payload = (await response.Content.ReadFromJsonAsync<CompanionFieldWorkOrderDetailResponse>())!;
    ...
    [Fact]
    public async Task Update_companion_field_work_order_status_proxies_to_maintainarr()
    {
        var token = await LoginAsync(PlatformSeeder.DemoAdminEmail);
        var request = Authorized(HttpMethod.Post, "/api/companion/field-tasks/work-order/status", token);
        request.Content = JsonContent.Create(new UpdateCompanionFieldWorkOrderStatusRequest(
        ...
        var payload = (await response.Content.ReadFromJsonAsync<CompanionFieldWorkOrderStatusResponse>())!;
    ...
    [Fact]
    public async Task Log_companion_field_work_order_labor_proxies_to_maintainarr()
    {
        var token = await LoginAsync(PlatformSeeder.DemoAdminEmail);
        var request = Authorized(HttpMethod.Post, "/api/companion/field-tasks/work-order/labor", token);
        request.Content = JsonContent.Create(new LogCompanionFieldWorkOrderLaborRequest(
            $"maintainarr:work-order:{_workOrderId:D}",
            1.5m,
            "regular",
            "Field labor note",
        ...
        var payload = (await response.Content.ReadFromJsonAsync<CompanionFieldWorkOrderLaborResponse>())!;
