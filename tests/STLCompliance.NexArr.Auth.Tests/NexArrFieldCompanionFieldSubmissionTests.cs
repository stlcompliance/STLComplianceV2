...
public sealed class NexArrFieldCompanionFieldSubmissionTests : IAsyncLifetime
{
    ...
    {
        var dbName = $"NexArrFieldCompanionSubmission-{Guid.NewGuid():N}";
    ...
    public async Task Offline_sync_records_acknowledge_submission_status()
    {
        ...
        var statusResponse = await _client.SendAsync(
            Authorized(
                HttpMethod.Get,
                $"/api/fieldcompanion/field-tasks/submission-status?taskKeys={Uri.EscapeDataString(taskKey)}",
                token));
        ...
        Assert.Equal(FieldCompanionFieldSubmissionKinds.Acknowledge, acknowledge.SubmissionKind);
        Assert.Equal(FieldCompanionFieldSubmissionStatuses.Synced, acknowledge.Status);
    }
    ...
    public async Task Submission_status_requires_task_keys()
    {
        ...
        var response = await _client.SendAsync(
            Authorized(HttpMethod.Get, "/api/fieldcompanion/field-tasks/submission-status", token));
    ...
    private static async Task EnsureFieldCompanionEntitlementAsync(NexArrDbContext db)
    {
        if (await db.Entitlements.AnyAsync(x =>
                x.TenantId == PlatformSeeder.DemoTenantId && x.ProductKey == "fieldcompanion"))
        {
            return;
        }
