...
public sealed class NexArrFieldCompanionFieldValidationTests : IAsyncLifetime
{
    ...
    {
        var dbName = $"NexArrFieldCompanionValidate-{Guid.NewGuid():N}";
    ...
    public async Task Validate_allows_acknowledge_for_inbox_task()
    {
        ...
        var response = await _client.SendAsync(Authorized(
            HttpMethod.Post,
            "/api/fieldcompanion/field-tasks/validate",
            token,
            new ValidateFieldCompanionFieldTaskRequest(taskKey, FieldCompanionFieldSubmissionKinds.Acknowledge, "trainarr")));

        response.EnsureSuccessStatusCode();
        var payload = (await response.Content.ReadFromJsonAsync<ValidateFieldCompanionFieldTaskResponse>())!;
    ...
    public async Task Validate_denies_task_not_in_inbox_with_plain_message()
    {
        ...
        var response = await _client.SendAsync(Authorized(
            ...
            "/api/fieldcompanion/field-tasks/validate",
            token,
            new ValidateFieldCompanionFieldTaskRequest(
        ...
        Assert.Equal(FieldCompanionFieldValidationReasonCodes.NotInInbox, payload.ReasonCode);
    ...
    public async Task Sync_validates_task_exists_in_inbox()
    {
        ...
        response.EnsureSuccessStatusCode();
        var synced = (await response.Content.ReadFromJsonAsync<SyncFieldCompanionOfflineActionsResponse>())!;
