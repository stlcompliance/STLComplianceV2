...
public sealed class NexArrFieldCompanionOfflineSyncTests : IAsyncLifetime
{
    ...
    {
        var dbName = $"NexArrFieldCompanionOffline-{Guid.NewGuid():N}";
    ...
    public async Task Sync_accepts_field_inbox_acknowledge_with_idempotency()
    {
        ...
        var syncRequest = Authorized(HttpMethod.Post, "/api/fieldcompanion/offline-actions/sync", token);
        syncRequest.Content = JsonContent.Create(new SyncFieldCompanionOfflineActionsRequest(
        [
            new FieldCompanionOfflineActionItem(
                idempotencyKey,
                FieldCompanionOfflineActionKinds.FieldInboxAcknowledge,
        ...
        var synced = (await syncResponse.Content.ReadFromJsonAsync<SyncFieldCompanionOfflineActionsResponse>())!;
        Assert.Equal(1, synced.Accepted);
        Assert.Equal(0, synced.Duplicates);

        var duplicateRequest = Authorized(HttpMethod.Post, "/api/fieldcompanion/offline-actions/sync", token);
        duplicateRequest.Content = JsonContent.Create(new SyncFieldCompanionOfflineActionsRequest(
        [
            new FieldCompanionOfflineActionItem(
                idempotencyKey,
                FieldCompanionOfflineActionKinds.FieldInboxAcknowledge,
        ...
        var duplicate = (await duplicateResponse.Content.ReadFromJsonAsync<SyncFieldCompanionOfflineActionsResponse>())!;
        Assert.Equal(0, duplicate.Accepted);
        Assert.Equal(1, duplicate.Duplicates);

        var listResponse = await _client.SendAsync(
            Authorized(HttpMethod.Get, "/api/fieldcompanion/offline-actions?limit=5", token));
        listResponse.EnsureSuccessStatusCode();
        var list = (await listResponse.Content.ReadFromJsonAsync<FieldCompanionOfflineActionsListResponse>())!;
    ...
    public async Task Sync_rejects_unsupported_action_kind()
    {
        ...
        var payload = (await response.Content.ReadFromJsonAsync<SyncFieldCompanionOfflineActionsResponse>())!;
        Assert.Equal(0, payload.Accepted);
        Assert.Equal(1, payload.Rejected);
        Assert.Equal("fieldcompanion.offline_actions.unsupported_kind", payload.RejectedItems[0].ReasonCode);
    }
    ...
    public async Task Sync_accepts_valid_actions_and_rejects_invalid_in_same_batch()
    {
        ...
        var response = await _client.SendAsync(Authorized(
            HttpMethod.Post,
            "/api/fieldcompanion/offline-actions/sync",
            token,
            new SyncFieldCompanionOfflineActionsRequest(
            [
                new FieldCompanionOfflineActionItem(
                    validKey,
                    FieldCompanionOfflineActionKinds.FieldInboxAcknowledge,
