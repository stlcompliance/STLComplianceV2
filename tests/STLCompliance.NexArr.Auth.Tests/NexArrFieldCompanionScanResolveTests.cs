...
public sealed class NexArrFieldCompanionScanResolveTests : IAsyncLifetime
{
    ...
    {
        var dbName = $"NexArrFieldCompanionScan-{Guid.NewGuid():N}";
    ...
    public async Task Resolve_scan_returns_matching_inbox_task()
    {
        ...
        var response = await _client.SendAsync(Authorized(
            HttpMethod.Post,
            "/api/fieldcompanion/scan/resolve",
            token,
            new FieldCompanionScanResolveRequest($"trainarr:assignment:{AssignmentId:D}", "qr")));

        response.EnsureSuccessStatusCode();
        var payload = (await response.Content.ReadFromJsonAsync<FieldCompanionScanResolveResponse>())!;

        Assert.Equal(FieldCompanionScanOutcomes.Resolved, payload.Outcome);
    ...
    public async Task Resolve_scan_denies_task_not_in_inbox()
    {
        ...
        var response = await _client.SendAsync(Authorized(
            ...
            "/api/fieldcompanion/scan/resolve",
            token,
            new FieldCompanionScanResolveRequest($"trainarr:assignment:{missingId:D}", null)));

        response.EnsureSuccessStatusCode();
        var payload = (await response.Content.ReadFromJsonAsync<FieldCompanionScanResolveResponse>())!;

        Assert.Equal(FieldCompanionScanOutcomes.Denied, payload.Outcome);
        Assert.Equal(FieldCompanionScanReasonCodes.NotInInbox, payload.ReasonCode);
    }
    ...
    public async Task Resolve_scan_requires_authentication()
    {
        var response = await _client.PostAsJsonAsync(
            "/api/fieldcompanion/scan/resolve",
            new FieldCompanionScanResolveRequest($"trainarr:assignment:{AssignmentId:D}", null));
    ...
    public async Task Resolve_scan_rejects_invalid_payload()
    {
        ...
        var response = await _client.SendAsync(Authorized(
            ...
            "/api/fieldcompanion/scan/resolve",
            token,
            new FieldCompanionScanResolveRequest("not-a-valid-scan", null)));
