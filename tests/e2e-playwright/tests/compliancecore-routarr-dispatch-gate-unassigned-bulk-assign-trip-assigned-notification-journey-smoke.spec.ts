...
import {
  ensureComplianceCoreRoutArrDispatchGateUnassignedBulkAssignTripAssignedNotificationFixture,
  issueRoutArrDispatchNotificationWorkerToken,
  loginNexArr,
  processRoutArrDispatchNotificationBatch,
  type ComplianceCoreRoutArrDispatchGateUnassignedBulkAssignTripAssignedNotificationFixture,
...
async function runComplianceCoreDispatchGateCheck(
  page: Page,
  fixture: ComplianceCoreRoutArrDispatchGateUnassignedBulkAssignTripAssignedNotificationFixture,
): Promise<void> {
...
async function overrideBulkAssignUnassignedWithGateDialog(
  page: Page,
  fixture: ComplianceCoreRoutArrDispatchGateUnassignedBulkAssignTripAssignedNotificationFixture,
): Promise<void> {
...
test.describe('Cross-product Compliance Core gate to RoutArr unassigned bulk assign trip-assigned notification @requires-live', () => {
  ...
  test('operator overrides gate block via unassigned bulk assign then trip_assigned notification appears in settings', async ({
    page,
  }, testInfo) => {
    ...
    let fixture: ComplianceCoreRoutArrDispatchGateUnassignedBulkAssignTripAssignedNotificationFixture
    try {
      fixture =
        await ensureComplianceCoreRoutArrDispatchGateUnassignedBulkAssignTripAssignedNotificationFixture()
    } catch {
      testInfo.skip(
        true,
        'Cross-product dispatch gate unassigned bulk assign trip-assigned notification fixture could not be created.',
      )
    }
    ...
    await overrideBulkAssignUnassignedWithGateDialog(page, fixture!)
