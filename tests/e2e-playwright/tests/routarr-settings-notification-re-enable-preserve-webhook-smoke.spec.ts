...
test.describe('RoutArr settings notification re-enable preserves webhook @requires-live', () => {
  ...
  test('re-enabling notifications reloads last saved webhook from API', async ({ page }) => {
    ...
    await seedRoutArrDispatchNotificationSettingsWithSavedWebhook()

    await page.reload()
    await expect(panel).toBeVisible({ timeout: 15_000 })
    await expect(enabled).toBeChecked()
    await expect(webhook).toHaveValue(routArrDispatchNotificationReEnablePreserveWebhookUrl)

    await assertRoutArrDispatchNotificationSettingsMatch({
      isEnabled: true,
      notificationWebhookUrl: routArrDispatchNotificationReEnablePreserveWebhookUrl,
    })
