...
test.describe('Companion field task evidence capture @requires-live', () => {
  ...
  test('uploads photo evidence for TrainArr assignment via companion API', async ({ page }, testInfo) => {
    if (!isLiveModeEnabled()) {
      testInfo.skip(true, 'Set E2E_LIVE=1 to run browser E2E against docker-compose.')
    }
    ...
    const evidencePanel = page.getByTestId('companion-field-evidence-panel').first()
    await expect(evidencePanel).toBeVisible({ timeout: 15_000 })

    await page.getByTestId('companion-evidence-kind-photo').first().click()
    await page.getByTestId('companion-evidence-file-input').first().setInputFiles({
      name: 'companion-e2e-photo.jpg',
      mimeType: 'image/jpeg',
      buffer: Buffer.from([0xff, 0xd8, 0xff, 0xd9]),
    })

    await page.getByTestId('companion-evidence-submit').first().click()
    await expect(page.getByTestId('companion-evidence-success').first()).toBeVisible({
      timeout: 20_000,
    })
  })
})
