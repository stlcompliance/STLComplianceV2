...
import {
  ensureComplianceCoreRoutArrDispatchGateTripAssignedNotificationFixture,
  issueRoutArrDispatchNotificationWorkerToken,
  loginNexArr,
  processRoutArrDispatchNotificationBatch,
  type ComplianceCoreRoutArrDispatchGateTripAssignedNotificationFixture,
...
async function runComplianceCoreDispatchGateCheck(
  page: Page,
  fixture: ComplianceCoreRoutArrDispatchGateTripAssignedNotificationFixture,
): Promise<void> {
...
test.describe('Cross-product Compliance Core gate to RoutArr trip-assigned notification @requires-live', () => {
  ...
  test('operator overrides gate block assign then trip_assigned notification appears in settings', async ({
    page,
  }, testInfo) => {
    ...
    let fixture: ComplianceCoreRoutArrDispatchGateTripAssignedNotificationFixture
    try {
      fixture = await ensureComplianceCoreRoutArrDispatchGateTripAssignedNotificationFixture()
    } catch {
      testInfo.skip(
        true,
        'Cross-product dispatch gate trip-assigned notification fixture could not be created.',
      )
    }
