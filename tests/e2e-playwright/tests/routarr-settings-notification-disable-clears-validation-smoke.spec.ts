...
test.describe('RoutArr settings notification disable clears validation @requires-live', () => {
  ...
  test('disabling notifications clears webhook validation without persisting invalid state', async ({
    page,
  }) => {
    ...
    const webhookBeforeDisableSave = initialWebhook.trim()
      ? initialWebhook.trim()
      : null
    await assertRoutArrDispatchNotificationSettingsMatch({
      isEnabled: false,
      notificationWebhookUrl: webhookBeforeDisableSave,
    })

    await assertRoutArrDispatchNotificationSettingsUpsertAcceptedWhenDisabled()
