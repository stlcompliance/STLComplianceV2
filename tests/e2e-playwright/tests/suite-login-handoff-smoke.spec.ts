...
test.describe('Suite frontend NexArr journey @requires-live', () => {
  ...
  test('login shows tenant name and slug in suite shell', async ({ page }) => {
    await signInFromSuite(page)

    await expect(page.getByTestId('suite-tenant-display-name')).toHaveText(
      demoTenant.displayName,
    )
    await expect(page.getByTestId('suite-tenant-slug')).toHaveText(demoTenant.slug)
    await expect(page.getByTestId('suite-user-display-name')).toContainText(
