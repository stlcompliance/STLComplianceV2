...
test.describe(
  'SupplyArr purchasing procurement exception investigate resolve journey @requires-live',
  () => {
    ...
    test.beforeAll(async ({}, testInfo) => {
      ...
      try {
        const fixture = await ensureSupplyArrProcurementExceptionInvestigateResolveJourneyFixture()
    ...
    test('handoff investigates then resolves open exception with resolution template', async ({
      page,
    }) => {
      test.skip(!hasJourneyFixture, 'Investigate/resolve journey fixture could not be seeded.')
