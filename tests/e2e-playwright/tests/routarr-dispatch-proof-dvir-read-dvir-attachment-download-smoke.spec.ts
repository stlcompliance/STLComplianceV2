...
import { ensureRoutArrDvirAttachmentDownloadFixture } from '../support/e2eApi.js'
...
test.describe('RoutArr dispatch proof/DVIR read DVIR attachment download @requires-live', () => {
  ...
  test.beforeAll(async ({}, testInfo) => {
    ...
    try {
      const fixture = await ensureRoutArrDvirAttachmentDownloadFixture()
  ...
  test('handoff loads dispatch proof/DVIR read panel and downloads DVIR attachment', async ({
    page,
  }) => {
    ...
    const attachmentButton = panel.getByTestId(`dvir-attachment-${fixtureAttachmentId}`)
    await expect(attachmentButton).toBeVisible({ timeout: 15_000 })
    await expect(attachmentButton).toContainText(/photo:/i)
