...
test.describe(
  'SupplyArr purchasing procurement exception investigate link PO resolve journey @requires-live',
  () => {
    ...
    test.beforeAll(async ({}, testInfo) => {
      ...
      try {
        const fixture =
          await ensureSupplyArrProcurementExceptionInvestigateLinkPoResolveJourneyFixture()
    ...
    test('handoff investigates, links follow-up PO, then resolves with template', async ({
      page,
    }) => {
      test.skip(
        !hasJourneyFixture,
        'Investigate/link PO/resolve journey fixture could not be seeded.',
      )
      ...
      await assertSupplyArrProcurementExceptionInvestigateLinkPoResolvedFromHandoff(
        openExceptionId,
