...
test.describe('RoutArr document attachment journey @requires-live', () => {
  ...
  test('handoff uploads pickup BOL document on driver portal then downloads it on dispatch read panel', async ({
    page,
  }) => {
    ...
    const attachmentButton = readPanel
      ...
      .filter({ hasText: new RegExp(`document:\\s*${fixtureFileName.replace(/[.*+?^${}()|[\]\\]/g, '\\$&')}`, 'i') })
