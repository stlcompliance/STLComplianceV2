...
import { ensureRoutArrTripCloseAfterCompleteFixture } from '../support/e2eApi.js'
...
test.describe('RoutArr trip close after complete journey @requires-live', () => {
  ...
  test.beforeAll(async ({}, testInfo) => {
    ...
    try {
      const fixture = await ensureRoutArrTripCloseAfterCompleteFixture()
  ...
  test('handoff completes trip then closes after capture and verifies dispatch read panel', async ({
    page,
  }) => {
    ...
    const completeButton = tripCard.getByRole('button', { name: 'Complete' })
    ...
    await expect(tripCard).toContainText(/completed/i, { timeout: 20_000 })
    await expect(tripCard.getByRole('button', { name: 'Complete' })).toHaveCount(0)

    const closeButton = tripCard.getByRole('button', { name: 'Close' })
    await expect(closeButton).toBeVisible({ timeout: 15_000 })
    await closeButton.click()
    await expect(tripCard).toHaveCount(0, { timeout: 20_000 })
    ...
    const summaryHeader = readPanel.getByTestId('trip-execution-summary-header')
    await expect(summaryHeader).toBeVisible({ timeout: 15_000 })
    await expect(summaryHeader).toContainText(/status completed/i)
    await expect(summaryHeader).toContainText(/driver closed yes/i)
    await expect(readPanel.getByText(/post DVIR yes/i)).toBeVisible({ timeout: 15_000 })
    await expect(readPanel.getByText(/DVIR inspections \(1\)/)).toBeVisible()
