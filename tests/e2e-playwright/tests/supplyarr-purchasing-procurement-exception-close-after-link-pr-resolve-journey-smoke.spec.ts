...
test.describe(
  'SupplyArr purchasing procurement exception close after link PR resolve journey @requires-live',
  () => {
    ...
    test.beforeAll(async ({}, testInfo) => {
      ...
      try {
        const fixture =
          await ensureSupplyArrProcurementExceptionCloseAfterLinkPrResolveJourneyFixture()
    ...
    test('handoff investigates, links follow-up PR, resolves with template, then closes', async ({
      page,
    }) => {
      test.skip(
        !hasJourneyFixture,
        'Close-after-link-PR-resolve journey fixture could not be seeded.',
      )
      ...
      await assertSupplyArrProcurementExceptionClosedAfterLinkPrResolveFromHandoff(
        openExceptionId,
