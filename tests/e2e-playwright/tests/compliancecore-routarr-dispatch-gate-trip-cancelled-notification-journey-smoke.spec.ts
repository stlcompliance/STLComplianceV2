...
import {
  ensureComplianceCoreRoutArrDispatchGateTripCancelledNotificationFixture,
  issueRoutArrDispatchNotificationWorkerToken,
  loginNexArr,
  processRoutArrDispatchNotificationBatch,
  type ComplianceCoreRoutArrDispatchGateTripCancelledNotificationFixture,
...
async function runComplianceCoreDispatchGateCheck(
  page: Page,
  fixture: ComplianceCoreRoutArrDispatchGateTripCancelledNotificationFixture,
): Promise<void> {
...
async function markTripCancelledViaBulkDispatch(
  page: Page,
  fixture: ComplianceCoreRoutArrDispatchGateTripCancelledNotificationFixture,
): Promise<void> {
  ...
  await panel.getByLabel('Dispatch status', { exact: false }).selectOption('cancelled')
...
test.describe('Cross-product Compliance Core gate to RoutArr trip-cancelled notification @requires-live', () => {
  ...
  test('operator overrides gate block assign, dispatches trip, marks cancelled, then trip_cancelled notification appears in settings', async ({
    page,
  }, testInfo) => {
    ...
    let fixture: ComplianceCoreRoutArrDispatchGateTripCancelledNotificationFixture
    try {
      fixture = await ensureComplianceCoreRoutArrDispatchGateTripCancelledNotificationFixture()
    } catch {
      testInfo.skip(
        true,
        'Cross-product dispatch gate trip-cancelled notification fixture could not be created.',
      )
    }
    ...
    await markTripCancelledViaBulkDispatch(page, fixture!)
    ...
    await expect(dispatchRow).not.toContainText('trip_completed')
