...
const resolutionTemplateKey = 'po_reissue'

test.describe(
  'SupplyArr purchasing procurement exception close after link PO resolve journey @requires-live',
  () => {
    let hasJourneyFixture = false
    let purchaseRequestId = ''
    let followUpPurchaseOrderId = ''
    let followUpOrderKey = ''
    let openExceptionId = ''
    ...
    test.beforeAll(async ({}, testInfo) => {
      ...
      try {
        const fixture =
          await ensureSupplyArrProcurementExceptionCloseAfterLinkPoResolveJourneyFixture()
        hasJourneyFixture = Boolean(fixture.openExceptionId)
        purchaseRequestId = fixture.purchaseRequestId
        followUpPurchaseOrderId = fixture.followUpPurchaseOrderId
        followUpOrderKey = fixture.followUpOrderKey
        openExceptionId = fixture.openExceptionId
      } catch {
        hasJourneyFixture = false
        purchaseRequestId = ''
        followUpPurchaseOrderId = ''
        followUpOrderKey = ''
        openExceptionId = ''
      }
    })

    test('handoff investigates, links follow-up PO, resolves with template, then closes', async ({
      page,
    }) => {
      test.skip(
        !hasJourneyFixture,
        'Close-after-link-PO-resolve journey fixture could not be seeded.',
      )
      ...
      await templateSelect.selectOption({ label: 'PO reissue' })
      ...
      await panel
        .getByTestId('procurement-exception-link-po')
        .selectOption(followUpPurchaseOrderId)
      ...
      await expect(linkedActions).toContainText(`PO ${followUpOrderKey}`)
      ...
      await assertSupplyArrProcurementExceptionClosedAfterLinkPoResolveFromHandoff(
        openExceptionId,
