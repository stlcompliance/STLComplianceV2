...
import {
  ensureComplianceCoreRoutArrDispatchGateTripInProgressNotificationFixture,
  issueRoutArrDispatchNotificationWorkerToken,
  loginNexArr,
  processRoutArrDispatchNotificationBatch,
  type ComplianceCoreRoutArrDispatchGateTripInProgressNotificationFixture,
...
async function runComplianceCoreDispatchGateCheck(
  page: Page,
  fixture: ComplianceCoreRoutArrDispatchGateTripInProgressNotificationFixture,
): Promise<void> {
...
async function markTripInProgressViaBulkDispatch(
  page: Page,
  fixture: ComplianceCoreRoutArrDispatchGateTripInProgressNotificationFixture,
): Promise<void> {
  ...
  await panel.getByLabel('Dispatch status', { exact: false }).selectOption('in_progress')
...
test.describe('Cross-product Compliance Core gate to RoutArr trip-in-progress notification @requires-live', () => {
  ...
  test('operator overrides gate block assign, dispatches trip, marks in progress, then trip_in_progress notification appears in settings', async ({
    page,
  }, testInfo) => {
    ...
    let fixture: ComplianceCoreRoutArrDispatchGateTripInProgressNotificationFixture
    try {
      fixture = await ensureComplianceCoreRoutArrDispatchGateTripInProgressNotificationFixture()
    } catch {
      testInfo.skip(
        true,
        'Cross-product dispatch gate trip-in-progress notification fixture could not be created.',
      )
    }
    ...
    await markTripInProgressViaBulkDispatch(page, fixture!)
