...
import { FieldCompanionFrontend, handoffProductFrontends, handoffUrlPattern } from '../support/productFrontends.js'

async function launchFieldCompanionFieldInbox(page: import('@playwright/test').Page) {
...
test.describe('Field Companion field inbox → operations product deep links @requires-live', () => {
  ...
  test('MaintainArr work order deep link opens workspace', async ({ page }, testInfo) => {
    ...
    await launchFieldCompanionFieldInbox(page)
