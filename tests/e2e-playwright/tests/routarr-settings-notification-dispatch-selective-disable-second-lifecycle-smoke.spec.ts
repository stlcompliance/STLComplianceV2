...
import {
  ...
  routArrDispatchNotificationUiSecondLifecycleSecondTripDisabledEventKinds,
...
const w287WebhookUrl = 'https://hooks.example.com/routarr-e2e-w287'
...
test.describe(
  'RoutArr settings dispatch notification selective disable second lifecycle smoke @requires-live',
  () => {
    ...
    test('all event kinds enabled then selective disable blocks second trip enqueues', async ({
      page,
    }) => {
      ...
      await enabled.check()
      await webhook.fill(w287WebhookUrl)
      ...
      await expect(webhook).toHaveValue(w287WebhookUrl)
      await expectEventToggleStates(panel, {
        tripAssigned: true,
        tripDispatched: true,
        tripInProgress: true,
      ...
      let secondTripId: string | null = null
      try {
        firstTripId = await createAndRunRoutArrDispatchNotificationFullLifecycle()
        ...
        secondTripId = await createAndRunRoutArrDispatchNotificationFullLifecycle()
        await assertRoutArrDispatchNotificationDispatchesForTrip(
          secondTripId,
          [routArrDispatchNotificationUiSecondLifecycleSecondTripEnabledEventKind],
          [...routArrDispatchNotificationUiSecondLifecycleSecondTripDisabledEventKinds],
      ...
      for (const eventKind of routArrDispatchNotificationUiSecondLifecycleFirstTripExpectedEventKinds) {
        const firstTripRow = dispatchList
          .locator('li')
          .filter({ hasText: firstTripId })
          .filter({ hasText: eventKind })
        await expect(firstTripRow).toBeVisible({ timeout: 15_000 })
        await expect(firstTripRow).toContainText(/pending/i)
      }

      const firstTripCancelledRow = dispatchList
        .locator('li')
        .filter({ hasText: firstTripId })
        .filter({ hasText: 'trip_cancelled' })
      await expect(firstTripCancelledRow).toHaveCount(0)

      const secondTripEnabledRow = dispatchList
        .locator('li')
        .filter({ hasText: secondTripId })
        .filter({ hasText: routArrDispatchNotificationUiSecondLifecycleSecondTripEnabledEventKind })
      await expect(secondTripEnabledRow).toBeVisible({ timeout: 15_000 })
      await expect(secondTripEnabledRow).toContainText(/pending/i)

      for (const eventKind of routArrDispatchNotificationUiSecondLifecycleSecondTripDisabledEventKinds) {
