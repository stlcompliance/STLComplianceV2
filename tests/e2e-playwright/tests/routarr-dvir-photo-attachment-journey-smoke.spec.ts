...
import { ensureRoutArrDvirPhotoAttachmentUploadFixture } from '../support/e2eApi.js'
...
test.describe('RoutArr DVIR photo attachment journey @requires-live', () => {
  ...
  test.beforeAll(async ({}, testInfo) => {
    ...
    try {
      const fixture = await ensureRoutArrDvirPhotoAttachmentUploadFixture()
  ...
  test('handoff submits pre-trip DVIR photo on driver portal then downloads it on dispatch read panel', async ({
    page,
  }) => {
    ...
    await expect(captureSection).toContainText(/pre DVIR/, { timeout: 20_000 })
    ...
    await expect(dvirAttachmentPanel).toContainText('pre trip DVIR attachments')
    ...
    await expect(readPanel.getByText(/pre DVIR/)).toBeVisible({ timeout: 15_000 })
