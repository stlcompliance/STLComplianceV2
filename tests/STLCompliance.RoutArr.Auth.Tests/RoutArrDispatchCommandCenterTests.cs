...
public sealed class RoutArrDispatchCommandCenterTests : IAsyncLifetime
{
    ...
    public async Task InitializeAsync()
    {
        ...
        var nexArrDbName = $"CmdCenterNexArr-{Guid.NewGuid():N}";
        var routArrDbName = $"CmdCenterRoutArr-{Guid.NewGuid():N}";
    ...
    [Fact]
    public async Task Command_center_returns_trip_columns_and_board()
    {
        ...
        var centerResponse = await _routarrClient.SendAsync(
            Authorized(HttpMethod.Get, "/api/dispatch/command-center?scope=daily", _dispatcherToken));
        centerResponse.EnsureSuccessStatusCode();

        var center = (await centerResponse.Content.ReadFromJsonAsync<DispatchCommandCenterResponse>())!;
        Assert.Equal("daily", center.Scope);
        Assert.Contains(center.TripColumns, x => x.DispatchStatus == "planned" && x.Count >= 1);
        Assert.NotNull(center.Board);
    }

    [Fact]
    public async Task Assign_driver_upserts_staffarr_person_ref()
    {
        var createRequest = Authorized(HttpMethod.Post, "/api/trips", _dispatcherToken);
        createRequest.Content = JsonContent.Create(new CreateTripRequest("Assign test", "", null, null, null, null));
        var createResponse = await _routarrClient.SendAsync(createRequest);
        createResponse.EnsureSuccessStatusCode();
        ...
        assignRequest.Content = JsonContent.Create(new AssignTripDriverRequest(
            personId,
            DriverDisplayName: "Alex Dispatcher"));

        var assignResponse = await _routarrClient.SendAsync(assignRequest);
        assignResponse.EnsureSuccessStatusCode();

        var refsResponse = await _routarrClient.SendAsync(
            Authorized(HttpMethod.Get, "/api/dispatch/driver-refs", _dispatcherToken));
        refsResponse.EnsureSuccessStatusCode();
        var refs = (await refsResponse.Content.ReadFromJsonAsync<StaffarrPersonRefListResponse>())!;
        Assert.Contains(refs.Items, x => x.PersonId == personId && x.DisplayName == "Alex Dispatcher");
    }

    [Fact]
    public async Task Board_state_persists_default_scope()
    {
        var putRequest = Authorized(HttpMethod.Put, "/api/dispatch/board-state", _dispatcherToken);
        putRequest.Content = JsonContent.Create(new UpsertDispatchBoardStateRequest("weekly"));
        (await _routarrClient.SendAsync(putRequest)).EnsureSuccessStatusCode();

        var getResponse = await _routarrClient.SendAsync(
            Authorized(HttpMethod.Get, "/api/dispatch/board-state", _dispatcherToken));
        getResponse.EnsureSuccessStatusCode();
        var state = (await getResponse.Content.ReadFromJsonAsync<DispatchBoardStateResponse>())!;
        Assert.Equal("weekly", state.DefaultScope);
    }
