...
public sealed class RoutArrEquipmentAvailabilityTests : IAsyncLifetime
{
    ...
    public async Task InitializeAsync()
    {
        ...
        var nexArrDbName = $"RoutArrEquipNexArr-{Guid.NewGuid():N}";
        var routArrDbName = $"RoutArrEquip-{Guid.NewGuid():N}";
    ...
    public async Task Equipment_availability_panel_returns_empty_summary_for_empty_tenant()
    {
        ...
        var panel = (await response.Content.ReadFromJsonAsync<EquipmentAvailabilityPanelResponse>())!;
    ...
    public async Task Equipment_availability_detects_conflict_with_assigned_trip()
    {
        ...
        createTripRequest.Content = JsonContent.Create(new CreateTripRequest(
            "Conflict trip",
            "Vehicle has overlapping assignment",
            vehicleRefKey,
            tripStart,
            tripEnd,
            null));
        ...
        var createAvailabilityRequest = Authorized(HttpMethod.Post, "/api/equipment-availability", dispatcherToken);
        createAvailabilityRequest.Content = JsonContent.Create(new CreateEquipmentAvailabilityRequest(
            vehicleRefKey,
            "unavailable",
        ...
        var availability = (await createAvailabilityResponse.Content.ReadFromJsonAsync<EquipmentAvailabilityDetailResponse>())!;
    ...
    [Fact]
    public async Task Driver_cannot_create_equipment_availability_record()
    {
        var driverToken = CreateRoutArrAccessToken(["routarr"], "routarr_driver", PlatformSeeder.DemoAdminUserId);
        var now = DateTimeOffset.UtcNow;

        var createRequest = Authorized(HttpMethod.Post, "/api/equipment-availability", driverToken);
