...
public static class CompanionScanPayloadParser
{
    ...
    public static bool TryExtractTaskKey(string scannedValue, out string taskKey, out string? errorMessage)
    {
        ...
        if (CompanionFieldTaskKeyParser.TryParse(trimmed, out _))
        {
            taskKey = trimmed;
            return true;
        }
        ...
        if (trimmed.StartsWith("stl-field-task:", StringComparison.OrdinalIgnoreCase))
        {
            ...
            if (CompanionFieldTaskKeyParser.TryParse(candidate, out _))
            {
                taskKey = candidate;
                return true;
            }
    ...
    private static bool TryParseJsonTaskKey(string trimmed, out string taskKey)
    {
        ...
        try
        {
            ...
            if (string.IsNullOrWhiteSpace(candidate) || !CompanionFieldTaskKeyParser.TryParse(candidate, out _))
            {
                return false;
            }
    ...
    private static bool TryParseUriTaskKey(Uri uri, out string taskKey)
    {
        ...
        if (!string.IsNullOrWhiteSpace(fromQuery) && CompanionFieldTaskKeyParser.TryParse(fromQuery, out _))
        {
            taskKey = fromQuery;
            return true;
        }
