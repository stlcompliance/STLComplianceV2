...
import { redeemHandoff, MaintainArrApiError } from '../api/client'
...
export function LaunchPage() {
  ...
  useEffect(() => {
    ...
    if (!handoff) {
      setError('Missing handoff code. Launch MaintainArr from the suite.')
      return
    }
    ...
    ;(async () => {
      try {
        ...
        if (!cancelled) {
          if (err instanceof MaintainArrApiError && err.status === 403) {
            setError('Your account is not entitled to MaintainArr for this tenant.')
            return
          }
          if (err instanceof MaintainArrApiError && err.status === 401) {
    ...
    <main className="flex min-h-screen items-center justify-center p-6">
      <div className="max-w-md rounded-xl border border-slate-700 bg-slate-900/80 p-8 text-center shadow-lg">
        {error ? (
          ...
          <p className="text-slate-300">Redeeming MaintainArr handoff…</p>
        )}
      </div>
    </main>
  )
