...
export function AuditPackageExportPanel({ accessToken, canExport }: AuditPackageExportPanelProps) {
  ...
  const zipExportMutation = useMutation({
    ...
    onSuccess: (blob) => {
      ...
      anchor.href = url
      anchor.download = `maintainarr-audit-package-${new Date().toISOString().slice(0, 10)}.zip`
      anchor.click()
  ...
  useEffect(() => {
    ...
    void downloadAuditPackageGenerationJob(accessToken, activeJobId).then((blob) => {
      ...
      const anchor = document.createElement('a')
      anchor.href = url
      anchor.download = `maintainarr-audit-package-${job.packageId ?? 'export'}.zip`
      anchor.click()
      URL.revokeObjectURL(url)
    })
    ...
    <section className="space-y-4 rounded-xl border border-slate-700 bg-slate-900/80 p-5">
      <header>
        ...
        <p className="mt-1 text-sm text-slate-400">
          Export tenant audit events, assets, work orders, defects, inspection runs, and PM schedule
          snapshots for maintenance compliance review. Date filters apply to time-bounded sections.
        </p>
      </header>
      ...
      {canExport ? (
        ...
        <p className="text-sm text-slate-500">
          Audit package export requires tenant admin, MaintainArr admin, or MaintainArr manager role.
        </p>
      )}
      ...
      {jobStatus ? (
        ...
        >
          ...
          {jobStatus.packageId ? (
            <p className="mt-1">
              Package <span className="font-mono text-amber-300">{jobStatus.packageId}</span>
      ...
      {lastJsonExport ? (
        <div className="rounded-lg border border-slate-800 bg-slate-950/50 p-4 text-sm text-slate-300">
          <p>
            ...
            {lastJsonExport.counts.auditEvents} audit events · {lastJsonExport.counts.assets} assets
            · {lastJsonExport.counts.workOrders} work orders · {lastJsonExport.counts.defects}{' '}
            defects · {lastJsonExport.counts.inspectionRuns} inspection runs ·{' '}
            {lastJsonExport.counts.pmSchedules} PM schedules
          </p>
        </div>
      ) : null}
