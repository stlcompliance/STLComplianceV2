...
describe('OrphanReferenceSettingsPanel', () => {
  ...
  it('shows retry callout when settings fail', async () => {
    vi.mocked(client.getOrphanReferenceSettings).mockRejectedValue(new Error('settings down'))
    vi.mocked(client.getOrphanReferenceFindings).mockResolvedValue({ items: [] })
    vi.mocked(client.getOrphanReferenceRuns).mockResolvedValue({ items: [] })

    renderPanel()

    expect(await screen.findByText('Orphan reference settings unavailable')).toBeInTheDocument()
