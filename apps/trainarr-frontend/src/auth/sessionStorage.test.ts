...
describe('trainarr sessionStorage', () => {
  const sampleSession: HandoffSessionResponse = {
    ...
    launchableProductKeys: ['trainarr'],
  ...
  it('removes the stored session when cleared', () => {
    ...
    expect(sessionStorageMock.removeItem).toHaveBeenCalledWith('stl.trainarr.session')
  })
})
