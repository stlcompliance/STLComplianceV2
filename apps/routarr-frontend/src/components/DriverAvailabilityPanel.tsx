    ...
    return (
      <li className={`rounded-lg border p-3 ${highlightClass}`}>
        <ConfirmDialog
          ...
          description="Delete this driver availability window? Dispatch may treat the driver as available again."
          confirmLabel="Delete"
          cancelLabel="Cancel"
          danger
          onConfirm={() => {
  ...
  return (
    <section className="rounded-xl border border-slate-700 bg-slate-950/50 p-5">
      ...
      {panel.records.length === 0 ? (
        <p className="text-sm text-[var(--color-text-muted)]">No driver availability records in this window.</p>
      ) : (
