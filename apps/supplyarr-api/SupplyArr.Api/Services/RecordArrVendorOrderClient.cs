...
public sealed record RecordArrVendorOrderRecordCreateRequest(
    string Title,
    string Description,
    string RecordType,
    string DocumentType,
...
public sealed record RecordArrVendorOrderFileCreateRequest(
    string RecordId,
...
public sealed record RecordArrVendorOrderRecordResponse(
    string RecordId,
    string RecordNumber);

public sealed record RecordArrVendorOrderFileResponse(
    string FileId);

public sealed record RegisteredVendorOrderDocumentRecord(
    string RecordId,
    string RecordNumber,
    string FileId);

public sealed class RecordArrVendorOrderClient(
    HttpClient httpClient,
    IOptions<RecordArrClientOptions> options)
{
    public async Task<RegisteredVendorOrderDocumentRecord> RegisterDocumentAsync(
        RecordArrVendorOrderRecordCreateRequest recordRequest,
        RecordArrVendorOrderFileCreateRequest fileRequest,
        CancellationToken cancellationToken = default)
    {
        ...
        var record = (await createRecordResponse.Content
            .ReadFromJsonAsync<RecordArrVendorOrderRecordResponse>(cancellationToken))!;
        ...
        var file = (await createFileResponse.Content
            .ReadFromJsonAsync<RecordArrVendorOrderFileResponse>(cancellationToken))!;

        return new RegisteredVendorOrderDocumentRecord(record.RecordId, record.RecordNumber, file.FileId);
    }
}
