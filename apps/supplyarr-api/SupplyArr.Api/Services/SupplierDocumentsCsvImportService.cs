...
public sealed class SupplierDocumentsCsvImportService(
    SupplyArrDbContext db,
    SupplierComplianceDocumentService documents)
{
    private const string ImportType = "supplier_documents_csv";
    ...
    public async Task<SupplierDocumentsCsvImportResponse> ImportAsync(
        Guid tenantId,
        Guid actorUserId,
        SupplierDocumentsCsvImportRequest request,
        CancellationToken cancellationToken = default)
    {
        var issues = new List<SupplierDocumentsCsvImportIssue>();
    ...
    private static SupplierDocumentsCsvImportResponse BuildResponse(
        bool dryRun,
        int rowsRead,
    ...
    private static IReadOnlyList<ImportRow> Parse(string csv, List<SupplierDocumentsCsvImportIssue> issues)
    {
        if (string.IsNullOrWhiteSpace(csv))
        {
            issues.Add(new SupplierDocumentsCsvImportIssue(1, "csv.empty", "CSV content is required."));
            return [];
        }
        ...
        {
            issues.Add(new SupplierDocumentsCsvImportIssue(1, "csv.header", $"Header must be: {string.Join(",", Headers)}."));
            return [];
        }
        ...
        for (var index = 1; index < lines.Length; index++)
        {
            ...
            {
                issues.Add(new SupplierDocumentsCsvImportIssue(
                    index + 1,
                    "csv.columns",
    ...
    private static void ValidateRow(
        ...
        List<SupplierDocumentsCsvImportIssue> issues)
    {
        ...
        {
            issues.Add(new SupplierDocumentsCsvImportIssue(row.LineNumber, "supplier.not_found", "Supplier key was not found."));
        }

        if (!seenDocumentKeys.Add($"{row.SupplierKey}|{row.DocumentKey}"))
        {
            issues.Add(new SupplierDocumentsCsvImportIssue(row.LineNumber, "document.duplicate_in_file", "Document key appears more than once for the same supplier in the import file."));
        }
    }
        ...
        List<SupplierDocumentsCsvImportIssue> issues)
    {
        ...
        issues.Add(new SupplierDocumentsCsvImportIssue(lineNumber, "csv.date", $"{column} must be a date or timestamp."));
        return null;
    }
        ...
        List<SupplierDocumentsCsvImportIssue> issues)
    {
        ...
        issues.Add(new SupplierDocumentsCsvImportIssue(lineNumber, "csv.integer", $"{column} must be a non-negative integer."));
        return 0;
    }
