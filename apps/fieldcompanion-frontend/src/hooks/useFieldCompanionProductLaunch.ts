...
import { buildFieldCompanionProductCallbackUrl } from '../lib/productLaunch'

export function useFieldCompanionProductLaunch(input: {
  accessToken: string
  suiteHomeUrl: string
  ...
  return useMutation({
    mutationFn: async (productKey: string) => {
      ...
      const callbackUrl = buildFieldCompanionProductCallbackUrl(
        normalized,
        input.suiteHomeUrl,
