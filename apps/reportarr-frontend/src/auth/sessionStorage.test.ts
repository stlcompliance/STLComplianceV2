...
describe('reportarr sessionStorage', () => {
  const sampleSession: ReportArrHandoffSessionResponse = {
  ...
  it('removes the stored session when cleared', () => {
    ...
    expect(sessionStorageMock.removeItem).toHaveBeenCalledWith('stl.reportarr.session')
  })
})
