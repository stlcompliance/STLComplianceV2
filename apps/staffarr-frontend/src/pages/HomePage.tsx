...
import { getMe, getOrgUnits, getPeople, getPerson, StaffArrApiError } from '../api/client'
...
export function HomePage() {
  ...
  return (
    <main className="mx-auto max-w-6xl p-8">
      ...
      <section className="mt-6 rounded-xl border border-slate-700 bg-slate-900/60 p-6">
        <h2 className="text-sm font-medium text-slate-300">Org units</h2>
        {orgUnits.length === 0 ? (
          <p className="mt-4 text-sm text-slate-400">No org units configured yet.</p>
        ) : (
          <ul className="mt-4 divide-y divide-slate-700">
            {orgUnits.map((unit) => (
              <li key={unit.orgUnitId} className="flex items-center justify-between py-2 text-sm">
                <span className="text-white">{unit.name}</span>
                <span className="text-xs uppercase tracking-wide text-slate-500">{unit.unitType}</span>
              </li>
            ))}
          </ul>
        )}
