...
describe('offlineSyncOutcome', () => {
  ...
  it('partitions retryable inbox failures', () => {
    const { retryableKeys, permanentKeys } = partitionRejectedItems([
      {
        idempotencyKey: 'a',
        reasonCode: 'companion.field_task.inbox_unavailable',
        reasonMessage: 'Try again later.',
      },
      {
        idempotencyKey: 'b',
        reasonCode: 'companion.field_task.not_in_inbox',
    ...
    expect(isRetryableOfflineRejection('companion.field_task.inbox_unavailable')).toBe(true)
  })
})
