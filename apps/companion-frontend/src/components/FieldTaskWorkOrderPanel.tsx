...
import {
  ...
  logCompanionFieldWorkOrderLabor,
  updateCompanionFieldWorkOrderStatus,
...
export function FieldTaskWorkOrderPanel({
  ...
  const detailQuery = useQuery({
    queryKey: ['companion-field-work-order', accessToken, task.taskKey],
    queryFn: async () => {
      ...
      return getCompanionFieldWorkOrderDetail(accessToken, task.taskKey)
    },
  ...
  const statusMutation = useMutation({
    mutationFn: async (status: string) => {
      ...
      return updateCompanionFieldWorkOrderStatus(accessToken, {
        taskKey: task.taskKey,
        status,
      })
    },
    ...
    onError: (error) => {
      const message = companionPlainReason(error, 'Work order status update failed.')
      setLocalSubmission({
        taskKey: task.taskKey,
        kind: 'work-order',
        phase: 'failed',
  ...
  const laborMutation = useMutation({
    mutationFn: async () => {
      ...
      return logCompanionFieldWorkOrderLabor(accessToken, {
        taskKey: task.taskKey,
        hoursWorked: hours,
        laborTypeKey,
    ...
    onError: (error) => {
      const message = companionPlainReason(error, 'Work order labor logging failed.')
      setLocalSubmission({
        taskKey: task.taskKey,
        kind: 'work-order',
        phase: 'failed',
  ...
  return (
    ...
    >
          ...
          <ul className="mt-2 space-y-2">
            {detail.tasks.map((taskLine) => (
              <li
                ...
                data-testid={`companion-work-order-task-${taskLine.sortOrder}`}
              >
                {taskLine.title}
        ...
        <div className="mt-4 space-y-3">
          <label className="block text-sm text-slate-200" htmlFor="companion-work-order-labor-hours">
            ...
            <input
              ...
              data-testid="companion-work-order-labor-hours"
            />
          </label>
          ...
          <label className="block text-sm text-slate-200" htmlFor="companion-work-order-labor-type">
            ...
            <select
              ...
              data-testid="companion-work-order-labor-type"
            >
          ...
          <label className="block text-sm text-slate-200" htmlFor="companion-work-order-labor-notes">
            ...
            <textarea
              ...
              data-testid="companion-work-order-labor-notes"
            />
          </label>
        </div>
      )}
      ...
      <div className="mt-4 flex flex-wrap gap-2">
        {editable && (
          <button
            ...
            data-testid="companion-work-order-log-labor"
            onClick={() => {
        ...
        {statusAction && statusActionLabel && (
          <button
            ...
            data-testid="companion-work-order-update-status"
            onClick={() => {
              setSuccessMessage(null)
      ...
      {successMessage && (
        <p className="mt-2 text-sm text-emerald-300" data-testid="companion-work-order-success">
          {successMessage}
        </p>
      )}
    </div>
