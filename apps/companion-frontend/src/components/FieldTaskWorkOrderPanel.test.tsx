...
vi.mock('../api/client', () => ({
  ...
  getCompanionFieldWorkOrderDetail: vi.fn(async () => workOrderDetail),
  updateCompanionFieldWorkOrderStatus: vi.fn(async () => ({
    taskKey: workOrderTask.taskKey,
    productKey: 'maintainarr',
  ...
  logCompanionFieldWorkOrderLabor: vi.fn(async () => ({
    taskKey: workOrderTask.taskKey,
    productKey: 'maintainarr',
    workOrderId: workOrderDetail.workOrderId,
...
describe('FieldTaskWorkOrderPanel', () => {
  ...
  it('renders work order detail for maintainarr work order tasks', async () => {
    ...
    expect(await screen.findByTestId('companion-field-work-order-panel')).toBeInTheDocument()
    expect(await screen.findByText('Remove old seal')).toBeInTheDocument()
    expect(await screen.findByTestId('companion-work-order-log-labor')).toBeInTheDocument()
  })

  it('logs labor and updates status through companion API', async () => {
    const client = new QueryClient({ defaultOptions: { queries: { retry: false } } })
    const { logCompanionFieldWorkOrderLabor, updateCompanionFieldWorkOrderStatus } =
    ...
    fireEvent.change(screen.getByTestId('companion-work-order-labor-hours'), {
      target: { value: '1.5' },
    })
    fireEvent.click(screen.getByTestId('companion-work-order-log-labor'))

    await waitFor(() => {
      expect(logCompanionFieldWorkOrderLabor).toHaveBeenCalledWith(
        'test-token',
    ...
    fireEvent.click(screen.getByTestId('companion-work-order-update-status'))

    await waitFor(() => {
      expect(updateCompanionFieldWorkOrderStatus).toHaveBeenCalledWith('test-token', {
        taskKey: workOrderTask.taskKey,
        status: 'in_progress',
      })
    })
