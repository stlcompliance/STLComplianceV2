...
export function NotificationSettingsPanel({ accessToken, canManage }: NotificationSettingsPanelProps) {
  ...
  const settingsQuery = useQuery({
    ...
    queryFn: () => getCompanionNotificationSettings(accessToken),
    enabled: canManage,
  })

  const dispatchesQuery = useQuery({
    queryKey: ['companion-notification-dispatches', accessToken],
    queryFn: () => getCompanionNotificationDispatches(accessToken, 5),
    enabled: canManage,
  })
  ...
  const saveMutation = useMutation({
    mutationFn: () =>
      upsertCompanionNotificationSettings(accessToken, {
        isEnabled,
    ...
    onSuccess: () => {
      void queryClient.invalidateQueries({ queryKey: ['companion-notification-settings', accessToken] })
      void queryClient.invalidateQueries({ queryKey: ['companion-notification-dispatches', accessToken] })
    },
  })
