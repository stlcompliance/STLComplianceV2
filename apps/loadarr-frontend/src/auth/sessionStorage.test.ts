...
describe('loadarr sessionStorage', () => {
  const sampleSession: LoadArrHandoffSessionResponse = {
  ...
  it('removes the stored session when cleared', () => {
    ...
    expect(sessionStorageMock.removeItem).toHaveBeenCalledWith('stl.loadarr.session')
  })
})
