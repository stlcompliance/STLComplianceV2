...
import { formatProductLaunchError, resolveProductLaunchCallbackPath } from '@stl/shared-ui'
...
export function LaunchPage() {
  ...
  useEffect(() => {
    ...
    if (!handoff) {
      setError('Missing handoff code. Launch CustomArr from the suite.')
      return
    }
  ...
  return (
    <main className="flex min-h-screen items-center justify-center p-6">
      <div className="customarr-card max-w-md p-8 text-center shadow-2xl">
        {error ? (
          <>
          ...
          <p className="text-slate-300">Redeeming CustomArr handoff…</p>
        )}
      </div>
    </main>
  )
