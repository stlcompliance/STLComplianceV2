...
describe('ordarr sessionStorage', () => {
  const sampleSession: OrdArrHandoffSessionResponse = {
    accessToken: 'access-token',
    accessTokenExpiresAt: new Date(Date.now() + 60_000).toISOString(),
    userId: 'user-1',
    personId: 'person-1',
    email: 'user@example.com',
    displayName: 'OrdArr User',
    tenantId: 'tenant-1',
    tenantSlug: 'tenant-one',
    tenantDisplayName: 'Tenant One',
    sessionId: 'session-1',
    tenantRoleKey: 'ordarr_ops',
  ...
  it('removes the stored session when cleared', () => {
    ...
    expect(sessionStorageMock.removeItem).toHaveBeenCalledWith('stl.ordarr.session')
  })
})
