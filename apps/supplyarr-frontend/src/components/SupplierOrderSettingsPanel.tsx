...
import { getSupplierOrderSettings, upsertSupplierOrderSettings } from '../api/supplierOrderClient'
...
export function SupplierOrderSettingsPanel({
  ...
  const mutation = useMutation({
    mutationFn: (payload: { allowDestinationSummaryInSupplierPortal: boolean; magicLinkTtlHours: number }) =>
    ...
    <section className="rounded-xl border border-slate-700 bg-slate-900/60 p-5" data-testid="supplier-order-settings-panel">
      ...
      <div className="mt-4 space-y-4">
        <label className="flex items-start gap-3 rounded-lg border border-slate-700 bg-slate-950/40 p-4 text-sm text-slate-300">
          <input
            type="checkbox"
            checked={settings.allowDestinationSummaryInSupplierPortal}
            onChange={(event) =>
              mutation.mutate({
                allowDestinationSummaryInSupplierPortal: event.target.checked,
        ...
        <label className="block text-sm text-slate-300">
          ...
          <input
            ...
            onChange={(event) =>
              mutation.mutate({
                allowDestinationSummaryInSupplierPortal: settings.allowDestinationSummaryInSupplierPortal,
