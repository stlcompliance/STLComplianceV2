...
import { syncSupplierCatalogApi } from '../api/client'
import type { PartResponse, SupplierCatalogApiSyncItem, SupplierCatalogApiSyncResponse } from '../api/types'
...
interface SupplierCatalogApiPanelProps {
  accessToken: string
  canManage: boolean
  parts: PartResponse[]
...
export function SupplierCatalogApiPanel({ accessToken, canManage, parts, suppliers }: SupplierCatalogApiPanelProps) {
  const queryClient = useQueryClient()
  const [selectedSupplierUnitId, setSelectedSupplierUnitId] = useState('')
  const [payloadJson, setPayloadJson] = useState('[]')
  const [dryRun, setDryRun] = useState(true)
  const [syncResult, setSyncResult] = useState<SupplierCatalogApiSyncResponse | null>(null)
  ...
  const currentEntries = useMemo<SupplierCatalogEntryRow[]>(
    () =>
      parts
        .flatMap((part) =>
  ...
  const syncMutation = useMutation({
    mutationFn: (vars: { supplierKey: string; items: SupplierCatalogApiSyncItem[] }) =>
      syncSupplierCatalogApi(accessToken, {
  ...
  const handleSync = () => {
    ...
    try {
      const parsed = JSON.parse(payloadJson) as SupplierCatalogApiSyncItem[]
      ...
      const normalizedItems = parsed.map((item) => ({
        ...
        supplierPartNumber: typeof item.supplierPartNumber === 'string' ? item.supplierPartNumber.trim() : '',
    ...
    <section className="rounded-xl border border-slate-700 bg-slate-900/60 p-5" data-testid="supplier-catalog-api-panel">
      ...
      <div className="mt-4 grid gap-4 lg:grid-cols-[minmax(0,320px)_minmax(0,1fr)]">
        ...
        <div className="space-y-3">
          <label htmlFor="supplier-catalog-api-payload" className="block text-sm text-slate-400">
            Supplier source JSON payload
            <textarea
              id="supplier-catalog-api-payload"
