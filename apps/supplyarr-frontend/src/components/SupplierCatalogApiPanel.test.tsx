...
import { SupplierCatalogApiPanel } from './SupplierCatalogApiPanel'
...
vi.mock('../api/client', async () => {
  ...
  return {
    ...actual,
    syncSupplierCatalogApi: vi.fn(),
  }
})

const mockSyncSupplierCatalogApi = vi.mocked(syncSupplierCatalogApi)
...
describe('SupplierCatalogApiPanel', () => {
  it('syncs a supplier source payload and refreshes the workspace caches', async () => {
    mockSyncSupplierCatalogApi.mockResolvedValue({
      syncType: 'supplier_catalog_api',
      dryRun: true,
      success: true,
      itemsRead: 1,
      itemsAccepted: 1,
    ...
    await waitFor(() => {
      expect(mockSyncSupplierCatalogApi).toHaveBeenCalledWith('token', {
        supplierKey: 'midwest-fleet',
        dryRun: true,
        items: [
          {
