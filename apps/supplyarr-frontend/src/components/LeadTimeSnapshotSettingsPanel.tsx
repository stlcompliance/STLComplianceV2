...
export function LeadTimeSnapshotSettingsPanel({ accessToken, canManage }: LeadTimeSnapshotSettingsPanelProps) {
  ...
  const settingsQuery = useQuery({
    queryKey: ['supplyarr-lead-time-snapshot-settings', accessToken],
    queryFn: () => getLeadTimeSnapshotSettings(accessToken),
    enabled: canManage,
  })

  const pendingQuery = useQuery({
    queryKey: ['supplyarr-lead-time-snapshot-pending', accessToken],
    queryFn: () => getPendingLeadTimeSnapshotCaptures(accessToken),
    enabled: canManage && isEnabled,
  })

  const runsQuery = useQuery({
    queryKey: ['supplyarr-lead-time-snapshot-runs', accessToken],
    queryFn: () => getLeadTimeSnapshotRuns(accessToken, 5),
    enabled: canManage,
  })
  ...
  const saveMutation = useMutation({
    mutationFn: () =>
      upsertLeadTimeSnapshotSettings(accessToken, {
        isEnabled,
        stalenessHours,
      }),
    onSuccess: () => {
      void queryClient.invalidateQueries({ queryKey: ['supplyarr-lead-time-snapshot-settings', accessToken] })
      void queryClient.invalidateQueries({ queryKey: ['supplyarr-lead-time-snapshot-pending', accessToken] })
      void queryClient.invalidateQueries({ queryKey: ['supplyarr-lead-time-snapshot-runs', accessToken] })
      void queryClient.invalidateQueries({ queryKey: ['supplyarr-lead-time-snapshots', accessToken] })
    },
  })
  ...
  return (
    <section
      ...
      data-testid="lead-time-snapshot-settings-panel"
    >
      ...
      <div className="mt-4 space-y-3">
        <label htmlFor="lead-time-snapshot-settings-enabled" className="flex items-center gap-2 text-sm text-slate-200">
          <input
            id="lead-time-snapshot-settings-enabled"
            type="checkbox"
            checked={isEnabled}
            onChange={(event) => setIsEnabled(event.target.checked)}
          />
          Enable automated lead-time snapshot capture
        </label>

        <label htmlFor="lead-time-snapshot-staleness-hours" className="block text-sm text-slate-200">
          <span className="font-medium">Staleness window (hours)</span>
          <input
            id="lead-time-snapshot-staleness-hours"
        ...
        >
          {saveMutation.isPending ? 'Saving…' : 'Save lead-time snapshot settings'}
        </button>
      ...
      <div className="mt-6">
        ...
        {pendingQuery.data && pendingQuery.data.items.length === 0 && (
          <p className="mt-2 text-sm text-slate-500">No vendor links currently due for lead-time snapshot capture.</p>
        )}
        {pendingQuery.data && pendingQuery.data.items.length > 0 && (
            ...
            {pendingQuery.data.items.map((item) => (
              <li key={item.partVendorLinkId} className="px-3 py-2 text-slate-300">
                ...
                <div className="text-xs text-slate-500">
                  ...
                  {item.currentLeadTimeDays != null
                    ? ` · current ${item.currentLeadTimeDays} days`
                    : ' · no current snapshot'}
                </div>
              </li>
            ))}
