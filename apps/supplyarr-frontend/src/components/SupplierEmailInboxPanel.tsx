...
import { getSupplierEmailInbox, ingestSupplierEmailInbox } from '../api/client'
import type { IngestSupplierEmailInboxResponse, SupplierEmailInboxMessageResponse } from '../api/types'
import { formatSupplierSummary } from '../utils/supplierPresentation'

interface SupplierEmailInboxPanelProps {
  accessToken: string
  canManage: boolean
}
...
function MessageCard({ message }: { message: SupplierEmailInboxMessageResponse }) {
  return (
...
export function SupplierEmailInboxPanel({ accessToken, canManage }: SupplierEmailInboxPanelProps) {
  ...
  const [lastResult, setLastResult] = useState<IngestSupplierEmailInboxResponse | null>(null)

  const inboxQuery = useQuery({
    queryKey: ['supplyarr-supplier-email-inbox', accessToken],
    queryFn: () => getSupplierEmailInbox(accessToken, 25),
    enabled: canManage,
  })

  const ingestMutation = useMutation({
    mutationFn: () =>
      ingestSupplierEmailInbox(accessToken, {
        messageKey,
        messageKind,
        senderEmail,
        senderName,
