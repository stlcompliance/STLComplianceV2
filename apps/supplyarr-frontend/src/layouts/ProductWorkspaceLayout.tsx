...
export function ProductWorkspaceLayout() {
  ...
  const sessionQuery = useQuery({
    queryKey: ['supplyarr-session', session?.accessToken],
  ...
  const launchCatalogQuery = useQuery({
    queryKey: ['supplyarr-launch-catalog', session?.accessToken],
    queryFn: () => getLaunchCatalog(apiBase, session!.accessToken, 'supplyarr'),
    enabled: Boolean(session?.accessToken),
    retry: false,
  })
