      ...
      <div className="mx-auto max-w-5xl space-y-6">
        <header className="rounded-[2rem] border border-amber-200 bg-[var(--color-bg-surface)] p-6 shadow-sm">
          <div className="flex flex-wrap items-start justify-between gap-4">
            ...
            <div className="rounded-2xl border border-amber-200 bg-amber-50 px-4 py-3 text-sm">
              <p className="font-semibold text-[var(--color-text-primary)]">{humanizeSupplierOrderValue(portal.status)}</p>
              <p className="mt-1 text-[var(--color-text-muted)]">Link expires {formatSupplierOrderDateTime(portal.linkExpiresAt)}</p>
            </div>
          </div>
        </header>
        ...
        <section className="grid gap-4 md:grid-cols-2 xl:grid-cols-4">
          <PortalCard
            ...
            value={humanizeSupplierOrderValue(portal.status)}
          ...
          <PortalCard
            ...
            value={formatSupplierOrderDateTime(portal.expectedReadyAt)}
            detail={`Confirmed ${formatSupplierOrderDateTime(portal.confirmedReadyAt)}`}
          />
          <PortalCard
            icon={<FileText className="h-5 w-5" />}
            label="Pickup window"
            value={formatSupplierOrderDateTime(portal.pickupWindowStart)}
            detail={`Until ${formatSupplierOrderDateTime(portal.pickupWindowEnd)}`}
          />
          <PortalCard
            icon={<CheckCircle2 className="h-5 w-5" />}
            label="Last update"
            value={formatSupplierOrderDateTime(latestStatusUpdate?.createdAt)}
            detail={latestStatusUpdate ? humanizeSupplierOrderValue(latestStatusUpdate.newStatus) : 'No updates yet'}
          />
        </section>
            ...
            <section className="rounded-[2rem] border border-[var(--color-border-subtle)] bg-[var(--color-bg-surface)] p-6 shadow-sm">
              ...
              <div className="mt-5 grid gap-4 md:grid-cols-2">
                <label className="text-sm text-[var(--color-text-secondary)]">
                  ...
                  >
                    {(metadata?.supplierPortalStatusOptions ?? []).map((status) => (
            ...
            <section className="rounded-[2rem] border border-[var(--color-border-subtle)] bg-[var(--color-bg-surface)] p-5 shadow-sm">
              ...
              <div className="mt-4 flex flex-wrap items-center gap-2">
                <span className={`inline-flex rounded-full border px-3 py-1 text-xs font-semibold ${
                  supplierOrderStatusTone(portal.status) === 'good'
                    ? 'border-emerald-200 bg-emerald-50 text-emerald-700'
                    : supplierOrderStatusTone(portal.status) === 'bad'
                      ? 'border-red-200 bg-red-50 text-red-700'
                      : 'border-amber-200 bg-amber-50 text-amber-700'
                }`}>
                  {humanizeSupplierOrderValue(portal.status)}
                </span>
              </div>
              <p className="mt-3 text-sm text-[var(--color-text-muted)]">
              ...
              {lastSavedAt ? (
                <p className="mt-3 text-sm text-emerald-700">
                  Update saved {formatSupplierOrderDateTime(lastSavedAt)}.
                </p>
              ) : null}
            </section>
                ...
                <ol className="mt-4 space-y-3">
                  {[...portal.statusHistory].reverse().slice(0, 5).map((entry) => (
                    <li key={entry.statusUpdateId} className="rounded-2xl border border-[var(--color-border-subtle)] bg-[var(--color-bg-surface-muted)] p-4">
                      <p className="font-medium text-[var(--color-text-primary)]">{humanizeSupplierOrderValue(entry.newStatus)}</p>
                      <p className="mt-1 text-xs text-[var(--color-text-muted)]">{formatSupplierOrderDateTime(entry.createdAt)}</p>
                ...
                <ul className="mt-4 space-y-3">
                  {portal.documents.map((document) => (
                    <li key={document.documentId} className="rounded-2xl border border-[var(--color-border-subtle)] bg-[var(--color-bg-surface-muted)] p-4">
                      ...
                      <p className="mt-1 text-xs text-[var(--color-text-muted)]">{humanizeSupplierOrderValue(document.documentType)}</p>
                    </li>
                  ))}
                </ul>
              ) : (
