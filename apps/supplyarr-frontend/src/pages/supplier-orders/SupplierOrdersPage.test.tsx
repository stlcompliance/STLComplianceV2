...
vi.mock('../../api/supplierOrderClient', () => ({
  getSupplierOrderMetadata: vi.fn().mockResolvedValue({
    filterStatusOptions: [
      { value: 'draft', label: 'Draft', owner: 'supplyarr', sourceOfTruth: 'supplyarr.supplier_order.workflow' },
...
describe('SupplierOrdersPage', () => {
  it('renders supplier orders with create and detail links', async () => {
    ...
    expect(screen.getByRole('link', { name: 'Open detail' })).toHaveAttribute(
      'href',
      '/purchasing/supplier-orders/supplier-order-1',
    )
  })
})
